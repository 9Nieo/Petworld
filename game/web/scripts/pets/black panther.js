...
class BlackPantherManager {
    constructor() {
        ...
        this.config = {
            ...
            animations: {
                left: '../../resources/actions/Black panther/left.png',
                leftAtlas: '../../resources/actions/Black panther/left.atlas'
            },
            // Animation settings
            animationSettings: {
                frameDuration: 22,      // Duration of each frame in milliseconds
                loopDelay: 0,           // Delay between animation loops in milliseconds
                totalFrames: 49,        // total frames (0-28)
                walkingFrames: {         // walking animation frame range
                    start: 0,            // walking animation start frame
                    end: 48             // walking animation end frame
                },
            ...
            shadow: {
                ...
                applyToExisting: true    // Whether to apply to existing black panthers
            }
        };
        ...
        // Track animated black panthers
        this.animatedBlackPanthers = new Map();
    ...
    /**
     * Initialize the Black Panther Manager
     */
    async init() {
        if (this.initialized) return;
        
        this.log('[BlackPantherManager] Initializing black panther animation system...');
        
        // Load and parse the atlas file
        try {
            await this.loadAtlasData();
        } catch (error) {
            this.error('[BlackPantherManager] Failed to load atlas data:', error);
        }
        
        // Listen for NFT loaded events to detect black panthers
        ...
        // Check for black panthers immediately if we're on the main page
        if (window.location.href.includes('home.html')) {
            // Short delay to ensure DOM is fully loaded
            setTimeout(() => this.checkForBlackPanthers(), 1000);
        }
        
        // Setup periodic black panther checking on main page
        this.setupPeriodicBlackPantherCheck();
    ...
    initShadowSystem() {
        ...
        this.log('[BlackPantherManager] Initializing shadow system...');
        
        // Check if PetShadowManager is available
        if (window.PetShadowManager) {
            this.log('[BlackPantherManager] PetShadowManager found, will use for black panther shadows');
            
            if (this.config.shadow.applyToExisting) {
                setTimeout(() => {
                    this.applyAllBlackPantherShadows();
                }, 1500);
            }
            
            document.addEventListener('black-panther-moving-state-changed', (event) => {
                if (event.detail && event.detail.element) {
                    const blackPantherElement = event.detail.element;
                    
                    // Update shadow state
                    if (window.PetShadowManager && window.PetShadowManager.updateShadow) {
                        window.PetShadowManager.updateShadow(blackPantherElement);
                    }
                }
            });
        } else {
            this.log('[BlackPantherManager] PetShadowManager not available, shadows will not be applied');
        }
    }
    
    applyAllBlackPantherShadows() {
        if (!window.PetShadowManager) return;
        
        this.log('[BlackPantherManager] Applying shadows to all black panthers...');
        
        const blackPantherElements = document.querySelectorAll('.black-panther-nft');
        let count = 0;
        
        blackPantherElements.forEach(blackPantherElement => {
            // Add shadow
            window.PetShadowManager.addShadow(blackPantherElement, {
                ratio: 0.8,  // Shadow width relative to black panther
                jumpEffect: true  // Enable jumping effect
            });
            count++;
        });
        
        this.log(`[BlackPantherManager] Applied shadows to ${count} black panthers`);
    }
    ...
    async loadAtlasData() {
        ...
        try {
            ...
            this.log(`[BlackPantherManager] Atlas path: ${atlasPath}`);
            ...
            this.log('[BlackPantherManager] Atlas data parsed using SpriteAtlasHelper');
            
            // get all image files to load
            const imageFiles = this.atlasData.meta.images.map(img => img.file);
            this.log(`[BlackPantherManager] Atlas references ${imageFiles.length} image files:`, imageFiles);
            ...
            this.log('[BlackPantherManager] Atlas data and all images loaded successfully');
        } catch (error) {
            this.error('[BlackPantherManager] Error loading atlas file:', error);
    ...
    async preloadAllAnimations(basePath, imageFiles) {
        try {
            ...
            this.log(`[BlackPantherManager] Successfully loaded ${Object.keys(this.loadedAnimations).length - 1} sprite sheets`);
        } catch (error) {
            this.error('[BlackPantherManager] Error preloading animations:', error);
    ...
    preloadAnimations() {
        ...
        if (!this.loadedAnimations || Object.keys(this.loadedAnimations).length === 0) {
            ...
            leftImg.onload = () => {
                this.log(`[BlackPantherManager] Main image preloaded successfully`);
                this.loadedAnimations = { default: leftImg, 0: leftImg };
            };
            leftImg.onerror = (err) => {
                this.error(`[BlackPantherManager] Error preloading main image:`, err);
            };
            
            // Get full path
            const imagePath = this.getAbsoluteResourcePath(this.config.animations.left);
            this.log(`[BlackPantherManager] Main image path: ${imagePath}`);
    ...
    /**
     * Observe the NFT container for new black panthers
     */
    observeNFTContainer() {
        ...
        const observer = new MutationObserver((mutations) => {
            let shouldCheckBlackPanthers = false;
            
            mutations.forEach(mutation => {
                if (mutation.type === 'childList' && mutation.addedNodes.length > 0) {
                    shouldCheckBlackPanthers = true;
                }
            });
            
            if (shouldCheckBlackPanthers) {
                this.checkForBlackPanthers();
            }
        });
        ...
        this.log('[BlackPantherManager] Now monitoring farm container for new NFTs');
    }
    ...
    createFallbackAtlasData() {
        this.log('[BlackPantherManager] Creating fallback atlas data');
    ...
    handleNFTLoadedEvent(event) {
        ...
        if (!nftElement.isConnected) {
            this.log(`[BlackPantherManager] NFT element not connected to DOM, skipping processing`);
            return;
        }
        ...
        if (this.processedNftIds.has(nftId)) {
            this.log(`[BlackPantherManager] NFT already processed, skipping: ${nftId}`);
            return;
        }
        
        // If element already has animation, skip
        if (this.animatedBlackPanthers.has(nftElement)) {
            this.log(`[BlackPantherManager] Element already animated, skipping: ${nftId}`);
            return;
        }
        
        // Check if this is a black panther
        if (this.isBlackPantherNFT(nftData)) {
            this.log('[BlackPantherManager] Black panther NFT detected from event:', nftData.metadata?.name || nftData.name || nftId);
            
            // Mark this NFT as processed
            this.processedNftIds.add(nftId);
            
            // Apply animation
            this.applyBlackPantherAnimation(nftElement);
        }
    }
    
    /**
     * Check the page for black panther NFTs and apply animations
     * @param {boolean} applyShadows - Whether to apply shadows, default is true
     */
    checkForBlackPanthers(applyShadows = true) {
        this.log('[BlackPantherManager] Checking for black panthers on the page...');
        
        // Find all NFT elements in the farm
        const farmContainer = document.getElementById('farm-animals-container');
        if (!farmContainer) {
            this.error('[BlackPantherManager] Farm container not found');
            return;
        }
        
        // Find all animal elements
        const animalElements = farmContainer.querySelectorAll('.animal');
        this.log(`[BlackPantherManager] Found ${animalElements.length} animal elements`);
        ...
        animalElements.forEach((animalElement, index) => {
            ...
            if (this.animatedBlackPanthers.has(animalElement)) {
                this.log(`[BlackPantherManager] Element already animated, skipping: ${nftId}`);
                return;
            }
            
            // Check if this is a black panther
            if (this.isBlackPantherNFTElement(animalElement)) {
                this.log(`[BlackPantherManager] Found black panther NFT #${index} (ID: ${nftId}):`, animalElement.dataset.name);
                
                // Mark this NFT as processed
                this.processedNftIds.add(nftId);
                
                // Apply animation
                this.applyBlackPantherAnimation(animalElement);
            }
        });
    ...
    isBlackPantherNFTElement(element) {
        // Check if the element has dataset
        if (!element.dataset) return false;
        
        // Check for black panther name, type, or class
        const name = element.dataset.name || '';
        const type = element.dataset.type || '';
        
        const hasBlackPantherName = name.toLowerCase().includes('black panther');
        const hasBlackPantherType = type.toLowerCase() === 'black panther';
        const hasBlackPantherClass = element.classList.contains('black-panther-nft') || element.classList.contains('black panther');
        
        return hasBlackPantherName || hasBlackPantherType || hasBlackPantherClass;
    }
    
    isBlackPantherNFT(nftData) {
        if (!nftData) return false;
        
        // Check metadata name
        if (nftData.metadata && nftData.metadata.name) {
            return nftData.metadata.name.toLowerCase().includes('black panther');
        }
        
        // Check direct name property
        if (nftData.name) {
            return nftData.name.toLowerCase().includes('black panther');
        }
        
        // Check any other properties that could identify a black panther
        if (nftData.type) {
            return nftData.type.toLowerCase() === 'black panther';
        }
        
        return false;
    }

    applyBlackPantherAnimation(element) {
        if (this.animatedBlackPanthers.has(element)) return;
        
        this.log('[BlackPantherManager] Applying black panther animation to element:', element);
        
        // Check if element is visible and has dimensions
        if (!element.isConnected) {
            this.error('[BlackPantherManager] Element is not connected to DOM, cannot animate:', element);
            return;
        }
    ...
    /**
     * Actual method to apply black panther animation (separated to support delayed processing)
     * @param {HTMLElement} element - The NFT element to animate
     */
    doApplyBlackPantherAnimation(element) {
        ...
        element.classList.add('black-panther-nft');
        
        this.animatedBlackPanthers.set(element, {
            element,
            isFlipped,
        ...
        if (!success) {
            this.error('[BlackPantherManager] Failed to prepare element for animation');
            return;
        }
        
        // start animation
        this.startBlackPantherAnimation(element, isFlipped);
        
        // apply shadow - ensure shadow is applied before setting movement
        if (this.config.shadow.enabled) {
            this.applyBlackPantherShadow(element);
        }
        ...
        this.updateBlackPantherDirection(element, isFlipped);
        
        this.log(`[BlackPantherManager] Black panther animation applied for: ${element.dataset.name || 'unnamed black panther'}`);
    }
    
    applyBlackPantherShadow(element) {
        if (!this.config.shadow.enabled || !window.PetShadowManager) return;
        
        // If already in tracking map, check if already has shadow
        const blackPantherData = this.animatedBlackPanthers.get(element);
        if (blackPantherData && blackPantherData.hasShadow) {
            this.log('[BlackPantherManager] Black panther already has shadow, skipping:', element.dataset.name || element.dataset.id || 'unnamed');
            return;
        }
        
        // Add black panther-nft class to support shadow selector
        element.classList.add('black-panther-nft');
        ...
        if (blackPantherData) {
            blackPantherData.hasShadow = true;
            this.animatedBlackPanthers.set(element, blackPantherData);
        }
        
        this.log('[BlackPantherManager] Applied shadow to black panther:', element.dataset.name || element.dataset.id || 'unnamed');
    }
    
    /**
     * Enable movement for black panthers
     * @param {HTMLElement} element - Black panther element
     */
    enableMovement(element) {
        if (!window.PetMovementManager) {
            this.log('[BlackPantherManager] PetMovementManager not available, cannot enable movement');
            return;
        }
        ...
        const positionObserver = setInterval(() => {
            ...
            if (Math.abs(deltaX) > 0.5) {
                ...
                this.updateBlackPantherDirection(element, isMovingRight);
                lastX = currentX;
            }
        }, 100); 
        
        const blackPantherData = this.animatedBlackPanthers.get(element) || {};
        blackPantherData.positionObserver = positionObserver;
        this.animatedBlackPanthers.set(element, blackPantherData);
        ...
        window.PetMovementManager.enableMovement(element, {
            ...
            onStateChange: (isMoving) => {
                ...
                const blackPantherEvent = new CustomEvent('black-panther-moving-state-changed', {
                    detail: {
                        element: element,
                        isMoving: isMoving
                    },
        ...
        const directionHandler = (event) => {
            if (event.detail && event.detail.element === element) {
                ...
                this.updateBlackPantherDirection(element, isRight);
            }
        };
        ...
        this.log(`[BlackPantherManager] Enabled movement for black panther: ${element.dataset.name || 'unnamed'} with speed ${speed.toFixed(2)}`);
    }
    ...
    prepareElementForAnimation(element, isFlipped) {
        ...
        try {
            ...
            const oldContainer = element.querySelector('.black-panther-animation-container');
            if (oldContainer) {
            ...
            animContainer.className = 'black-panther-animation-container';
            ...
            animContainer.style.zIndex = '10'; // Show above the black panther image
            ...
            if (shouldFlip) {
                this.log('[BlackPantherManager] Setting initial animation direction: right (flipped)');
                ...
                this.log('[BlackPantherManager] Setting initial animation direction: left (normal)');
            ...
            this.log(`[BlackPantherManager] Element dimensions: ${elementWidth}x${elementHeight}`); 
            
            // Create canvas for sprite animation
            const canvas = document.createElement('canvas');
            canvas.className = 'black-panther-animation-canvas';
            ...
            this.error('[BlackPantherManager] Error preparing element for animation:', error);
            return false;
        }
    }
    
    /**
     * Start the black panther animation
     * @param {HTMLElement} element - The element to animate
     * @param {boolean} isFlipped - Whether the animation is flipped
     */
    startBlackPantherAnimation(element, isFlipped) {
        // Check element
        if (!element || !element.isConnected) {
            this.error('[BlackPantherManager] Cannot start animation - element is not connected to DOM');
            return;
        }
        
        try {
            // Get animation container
            const animContainer = element.querySelector('.black-panther-animation-container');
            if (!animContainer) {
                this.error('[BlackPantherManager] Animation container not found');
                return;
            }
            
            // Get the canvas
            const canvas = animContainer.querySelector('.black-panther-animation-canvas');
            if (!canvas) {
                this.error('[BlackPantherManager] Canvas not found');
                return;
            }
            
            const blackPantherData = this.animatedBlackPanthers.get(element);
            if (!blackPantherData) {
                this.error('[BlackPantherManager] Black panther data not found');
                return;
            }
            
            // Wait until the atlas data is loaded
            if (!this.atlasData) {
                this.log('[BlackPantherManager] Atlas data not loaded yet, creating fallback');
                this.createFallbackAtlasData();
            }
            ...
            this.error('[BlackPantherManager] Error starting black panther animation:', error);
        }
    }
    ...
    startSpriteAnimation(element, canvas, isFlipped) {
        try {
            // Get black panther data from tracking map
            const blackPantherData = this.animatedBlackPanthers.get(element);
            if (!blackPantherData) return;
            
            // Get the canvas context
            const ctx = canvas.getContext('2d');
            if (!ctx) {
                this.error('[BlackPantherManager] Failed to get canvas context');
                return;
            }
            
            // check if sprite sheets are loaded
            if (!this.loadedAnimations || Object.keys(this.loadedAnimations).length === 0) {
                this.log('[BlackPantherManager] Sprite sheets not yet loaded, trying to load now...');
                
                // try to load main image from the path in config
                const leftImgPath = this.getAbsoluteResourcePath(this.config.animations.left);
                const img = new Image();
                img.onload = () => {
                    this.log('[BlackPantherManager] Main image loaded for animation');
                ...
                img.onerror = (err) => {
                    this.error('[BlackPantherManager] Failed to load image for animation:', err);
                };
                img.src = leftImgPath;
                return;
            }
            ...
            this.error('[BlackPantherManager] Error in startSpriteAnimation:', error);
        }
    }
    ...
    initializeAnimation(element, canvas, ctx, isFlipped) {
        try {
            ...
            if (!element || !element.isConnected) {
                this.error('[BlackPantherManager] Element no longer connected to DOM');
                return;
            }
            
            const blackPantherData = this.animatedBlackPanthers.get(element);
            if (!blackPantherData) {
                this.error('[BlackPantherManager] Black panther data not found in initializeAnimation');
                return;
            }
            
            // check if sprite sheets are loaded
            if (!this.loadedAnimations || Object.keys(this.loadedAnimations).length === 0) {
                this.error('[BlackPantherManager] Sprite sheets not loaded');
                return;
            }
            
            // at least one default image is required
            if (!this.loadedAnimations.default || !this.loadedAnimations.default.complete) {
                this.error('[BlackPantherManager] Default sprite sheet not loaded or incomplete');
                return;
            }
            
            // Clear any existing animation
            if (blackPantherData.animationInterval) {
                clearInterval(blackPantherData.animationInterval);
                blackPantherData.animationInterval = null;
            }
            ...
            if (canvas.width !== elementWidth * 1.2 || canvas.height !== elementHeight * 1.2) {
                this.log(`[BlackPantherManager] Updating canvas size to match element: ${elementWidth}x${elementHeight}`);
            ...
            const isMoving = element.classList.contains('pet-moving') || element.classList.contains('black-panther-moving');
            
            blackPantherData.currentFrame = 0;
            blackPantherData.animationState = isMoving ? 'walking' : 'idle';
            
            // immediately draw the first frame
            this.drawFrame(ctx, canvas, this.loadedAnimations, blackPantherData.currentFrame);
            
            // set animation interval
            const frameDuration = this.config.animationSettings.frameDuration;
            
            blackPantherData.animationInterval = setInterval(() => {
                // check if element is still connected to DOM
                if (!element.isConnected) {
                    if (blackPantherData.animationInterval) {
                        clearInterval(blackPantherData.animationInterval);
                ...
                const isCurrentlyMoving = element.classList.contains('pet-moving') || 
                                          element.classList.contains('black-panther-moving');
                
                // update animation based on current state
                if (isCurrentlyMoving) {
                    // when in moving state, play the complete walking animation
                    if (blackPantherData.animationState !== 'walking') {
                        // state changed from idle to moving, reset to the 0th frame
                        blackPantherData.currentFrame = 0;
                        blackPantherData.animationState = 'walking';
                    } else {
                        // loop within the walking frame range
                        const {start, end} = this.config.animationSettings.walkingFrames;
                        blackPantherData.currentFrame++;
                        
                        // loop back to the start frame
                        if (blackPantherData.currentFrame > end) {
                            blackPantherData.currentFrame = start;
                        }
                    }
                } else {
                    // when not in moving state, only display idle frame
                    if (blackPantherData.animationState !== 'idle') {
                        blackPantherData.currentFrame = this.config.animationSettings.idleFrames.start;
                        blackPantherData.animationState = 'idle';
                    }
                ...
                if (Math.abs(currentWidth - elementWidth) > elementWidth * 0.1 ||
                    ...
                    this.log(`[BlackPantherManager] Element size changed, updated canvas: ${canvas.width}x${canvas.height}`);
                }
            ...
            this.error('[BlackPantherManager] Error in initializeAnimation:', error);
        }
    }
    ...
    getCurrentRotationTestConfig() {
        ...
        if (now - this.rotationTestConfig.lastChangeTime > this.rotationTestConfig.timePerTest) {
            ...
            console.log(`[BlackPantherManager] Test rotation configuration #${this.rotationTestConfig.currentTest}: ${currentConfig.description}`);
        }
    ...
    drawFrame(ctx, canvas, spriteSheets, frameIndex) {
        ...
        if (!frameData) {
            this.error(`[BlackPantherManager] Frame data not found for: ${frameName}`);
            return;
        }
        ...
        try {
            ...
            this.error('[BlackPantherManager] Error drawing frame:', error);
        }
    }
    
    pauseAllAnimations() {
        this.animatedBlackPanthers.forEach((blackPantherData, element) => {
            if (blackPantherData.animationInterval) {
                clearInterval(blackPantherData.animationInterval);
                blackPantherData.animationInterval = null;
            }
    ...
    resumeAllAnimations() {
        this.animatedBlackPanthers.forEach((blackPantherData, element) => {
            if (!blackPantherData.animationInterval && element.isConnected) {
                // Restart animation
                const animContainer = element.querySelector('.black-panther-animation-container');
                if (animContainer) {
                    const canvas = animContainer.querySelector('.black-panther-animation-canvas');
                    if (canvas) {
                        this.startBlackPantherAnimation(element, blackPantherData.isFlipped);
                    }
                }
            }
        });
    ...
    cleanupNonExistentAnimations(currentNftIds) {
        ...
        elementsToRemove.forEach(element => {
            this.log(`[BlackPantherManager] Cleaning up animation for removed element: ${element.dataset.id || element.dataset.name}`);
    ...
    updateBlackPantherDirection(element, isRight) {
        this.log(`[BlackPantherManager] Updating black panther direction: ${isRight ? 'right' : 'left'}`);
        ...
        const container = element.querySelector('.black-panther-animation-container');
        if (container) {
            if (isRight) {
        ...
        const blackPantherData = this.animatedBlackPanthers.get(element);
        if (blackPantherData) {
            blackPantherData.isFlipped = isRight;
            this.animatedBlackPanthers.set(element, blackPantherData);
        }
    ...
    cleanupAnimation(element) {
        ...
        if (blackPantherData.positionObserver) {
            clearInterval(blackPantherData.positionObserver);
        }
        ...
        this.animatedBlackPanthers.delete(element);
    }
    ...
    cleanupAllAnimations() {
        this.animatedBlackPanthers.forEach((_, element) => {
            this.cleanupAnimation(element);
        });
    }
     ...
     */
    resetProcessedState() {
        this.log('[BlackPantherManager] Resetting processed state, will recheck all NFTs');
        this.processedNftIds.clear();
    ...
    setupDirectionChangeListener() {
        ...
        document.addEventListener('pet-direction-changed', (event) => {
            ...
            if (this.isBlackPantherNFTElement(element)) {
                this.updateBlackPantherDirection(element, direction === 'right');
            }
        });
        
        // if PetMovementManager is available, listen to its direction change event
        if (window.PetMovementManager) {
            this.log('[BlackPantherManager] Adding PetMovementManager direction change hook');
            ...
            if (typeof originalUpdateDirectionState === 'function') {
                window.PetMovementManager.updateDirectionState = (pet) => {
                    ...
                    // then check if it is a black panther and update direction
                    if (this.isBlackPantherNFTElement(pet.element)) {
                        // get direction information
                        const isFacingRight = pet.state.direction.x > 0;
                        
                        // delay update to ensure state has been updated by the original method
                        setTimeout(() => {
                            this.updateBlackPantherDirection(pet.element, isFacingRight);
                        }, 10);
                    }
                };
            }
...
// Create and initialize the black panther manager
const blackPantherManager = new BlackPantherManager();

// Initialize when DOM is ready
document.addEventListener('DOMContentLoaded', () => {
    blackPantherManager.init();
});

// Listen for NFT refresh events on the page
window.addEventListener('nftRefreshed', () => {
    if (blackPantherManager.initialized) {
        // When NFTs are refreshed, reset processed state and recheck
        blackPantherManager.resetProcessedState();
        // Delay slightly to ensure DOM has updated
        setTimeout(() => blackPantherManager.checkForBlackPanthers(), 500);
    }
});

// Export the black panther manager for use in other modules
window.BlackPantherManager = blackPantherManager;

// Add the backward compatibility method checkForBlackPanthers just like in egg.js
// First check if this is already using PetAnimationBase
if (BlackPantherManager.prototype instanceof PetAnimationBase) {
    /**
     * Backward compatibility method - calls checkForPets
     * @param {boolean} applyShadows - Whether to apply shadows
     */
    BlackPantherManager.prototype.checkForBlackPanthers = function(applyShadows = true) {
