// DragonManager class to handle all dragon-related functionality
class DragonManager {
    constructor() {
        ...
        this.config = {
            ...
            shadow: {
                ...
                applyToExisting: true    // Whether to apply to existing dragons
            }
        };
        ...
        // Track animated dragons
        this.animatedDragons = new Map();
    ...
    async init() {
        ...
        this.log('[DragonManager] Initializing dragon animation system...');
        
        // Load and parse the atlas file
        try {
            await this.loadAtlasData();
        } catch (error) {
            this.error('[DragonManager] Failed to load atlas data:', error);
        }
        
        // Listen for NFT loaded events to detect dragons
        ...
        // Check for dragons immediately if we're on the main page
        if (window.location.href.includes('home.html')) {
            // Short delay to ensure DOM is fully loaded
            setTimeout(() => this.checkForDragons(), 1000);
        }
        
        // Setup periodic dragon checking on main page
        this.setupPeriodicDragonCheck();
    ...
    initShadowSystem() {
        ...
        this.log('[DragonManager] Initializing shadow system...');
        
        // Check if PetShadowManager is available
        if (window.PetShadowManager) {
            this.log('[DragonManager] PetShadowManager found, will use for dragon shadows');
            
            if (this.config.shadow.applyToExisting) {
                setTimeout(() => {
                    this.applyAllDragonShadows();
                }, 1500);
            }
            
            document.addEventListener('dragon-moving-state-changed', (event) => {
                if (event.detail && event.detail.element) {
                    const dragonElement = event.detail.element;
                    
                    // Update shadow state
                    if (window.PetShadowManager && window.PetShadowManager.updateShadow) {
                        window.PetShadowManager.updateShadow(dragonElement);
                    }
                }
            });
        } else {
            this.log('[DragonManager] PetShadowManager not available, shadows will not be applied');
        }
    }
    
    applyAllDragonShadows() {
        if (!window.PetShadowManager) return;
        
        this.log('[DragonManager] Applying shadows to all dragons...');
        
        const dragonElements = document.querySelectorAll('.dragon-nft');
        let count = 0;
        
        dragonElements.forEach(dragonElement => {
            // Add shadow
            window.PetShadowManager.addShadow(dragonElement, {
                ratio: 0.8,  // Shadow width relative to dragon
                jumpEffect: true  // Enable jumping effect
            });
            count++;
        });
        
        this.log(`[DragonManager] Applied shadows to ${count} dragons`);
    }
    ...
    async loadAtlasData() {
        ...
        try {
            ...
            this.log(`[DragonManager] Atlas path: ${atlasPath}`);
            ...
            this.log('[DragonManager] Atlas data parsed using SpriteAtlasHelper');
            
            // get all image files to load
            const imageFiles = this.atlasData.meta.images.map(img => img.file);
            this.log(`[DragonManager] Atlas references ${imageFiles.length} image files:`, imageFiles);
            ...
            this.log('[DragonManager] Atlas data and all images loaded successfully');
        } catch (error) {
            this.error('[DragonManager] Error loading atlas file:', error);
    ...
    async preloadAllAnimations(basePath, imageFiles) {
        try {
            ...
            this.log(`[DragonManager] Successfully loaded ${Object.keys(this.loadedAnimations).length - 1} sprite sheets`);
        } catch (error) {
            this.error('[DragonManager] Error preloading animations:', error);
    ...
    preloadAnimations() {
        ...
        if (!this.loadedAnimations || Object.keys(this.loadedAnimations).length === 0) {
            ...
            leftImg.onload = () => {
                this.log(`[DragonManager] Main image preloaded successfully`);
                this.loadedAnimations = { default: leftImg, 0: leftImg };
            };
            leftImg.onerror = (err) => {
                this.error(`[DragonManager] Error preloading main image:`, err);
            };
            
            // Get full path
            const imagePath = this.getAbsoluteResourcePath(this.config.animations.left);
            this.log(`[DragonManager] Main image path: ${imagePath}`);
    ...
    setupPeriodicDragonCheck() {
        if (window.location.href.includes('home.html')) {
            setInterval(() => {
                this.checkForDragons(false); // Pass false to indicate not to auto-apply shadows
            }, 10000);
        }
    }
    ...
    /**
     * Observe the NFT container for new dragons
     */
    observeNFTContainer() {
        ...
        const observer = new MutationObserver((mutations) => {
            let shouldCheckDragons = false;
            
            mutations.forEach(mutation => {
                if (mutation.type === 'childList' && mutation.addedNodes.length > 0) {
                    shouldCheckDragons = true;
                }
            });
            
            if (shouldCheckDragons) {
                this.checkForDragons();
            }
        });
        ...
        this.log('[DragonManager] Now monitoring farm container for new NFTs');
    }
    ...
    createFallbackAtlasData() {
        this.log('[DragonManager] Creating fallback atlas data');
    ...
    handleNFTLoadedEvent(event) {
        ...
        if (!nftElement.isConnected) {
            this.log(`[DragonManager] NFT element not connected to DOM, skipping processing`);
            return;
        }
        ...
        if (this.processedNftIds.has(nftId)) {
            this.log(`[DragonManager] NFT already processed, skipping: ${nftId}`);
            return;
        }
        
        // If element already has animation, skip
        if (this.animatedDragons.has(nftElement)) {
            this.log(`[DragonManager] Element already animated, skipping: ${nftId}`);
            return;
        }
        
        // Check if this is a dragon
        if (this.isDragonNFT(nftData)) {
            this.log('[DragonManager] Dragon NFT detected from event:', nftData.metadata?.name || nftData.name || nftId);
            
            // Mark this NFT as processed
            this.processedNftIds.add(nftId);
            
            // Apply animation
            this.applyDragonAnimation(nftElement);
        }
    }
    
    /**
     * Check the page for dragon NFTs and apply animations
     * @param {boolean} applyShadows - Whether to apply shadows, default is true
     */
    checkForDragons(applyShadows = true) {
        this.log('[DragonManager] Checking for dragons on the page...');
        
        // Find all NFT elements in the farm
        const farmContainer = document.getElementById('farm-animals-container');
        if (!farmContainer) {
            this.error('[DragonManager] Farm container not found');
            return;
        }
        
        // Find all animal elements
        const animalElements = farmContainer.querySelectorAll('.animal');
        this.log(`[DragonManager] Found ${animalElements.length} animal elements`);
        ...
        animalElements.forEach((animalElement, index) => {
            ...
            if (this.animatedDragons.has(animalElement)) {
                this.log(`[DragonManager] Element already animated, skipping: ${nftId}`);
                return;
            }
            
            // Check if this is a dragon
            if (this.isDragonNFTElement(animalElement)) {
                this.log(`[DragonManager] Found dragon NFT #${index} (ID: ${nftId}):`, animalElement.dataset.name);
                
                // Mark this NFT as processed
                this.processedNftIds.add(nftId);
                
                // Apply animation
                this.applyDragonAnimation(animalElement);
            }
        });
    ...
    isDragonNFTElement(element) {
        // Check if the element has dataset
        if (!element.dataset) return false;
        
        // Check for dragon name, type, or class
        const name = element.dataset.name || '';
        const type = element.dataset.type || '';
        
        const hasDragonName = name.toLowerCase().includes('dragon');
        const hasDragonType = type.toLowerCase() === 'dragon';
        const hasDragonClass = element.classList.contains('dragon-nft') || element.classList.contains('dragon');
        
        return hasDragonName || hasDragonType || hasDragonClass;
    }
    ...
    isDragonNFT(nftData) {
        ...
        if (nftData.metadata && nftData.metadata.name) {
            return nftData.metadata.name.toLowerCase().includes('dragon');
        }
        
        // Check direct name property
        if (nftData.name) {
            return nftData.name.toLowerCase().includes('dragon');
        }
        
        // Check any other properties that could identify a dragon
        if (nftData.type) {
            return nftData.type.toLowerCase() === 'dragon';
        }
        
        return false;
    }

    applyDragonAnimation(element) {
        if (this.animatedDragons.has(element)) return;
        
        this.log('[DragonManager] Applying dragon animation to element:', element);
        
        // Check if element is visible and has dimensions
        if (!element.isConnected) {
            this.error('[DragonManager] Element is not connected to DOM, cannot animate:', element);
            return;
        }
        ...
        if (window.PetSizeHelper) {
            window.PetSizeHelper.applyAnimationWithSizeCheck(
                element,
                'dragon',
                (el) => this.doApplyDragonAnimation(el),
                {
                    maxWaitAttempts: 8,
            ...
            this.log('[DragonManager] PetSizeHelper not available, using fallback logic');
            ...
            if (effectiveWidth === 0 || effectiveHeight === 0) {
                this.log('[DragonManager] Element has zero effective dimensions, delaying animation:', element);
                ...
                setTimeout(() => {
                    ...
                    if (element.isConnected && (newEffectiveWidth > 0 && newEffectiveHeight > 0)) {
                        this.log('[DragonManager] Element now has dimensions, applying animation');
                        this.doApplyDragonAnimation(element);
                    } else {
                        this.error('[DragonManager] Element still has zero dimensions after delay, applying PetSizeManager or default size');
                        ...
                        if (window.PetSizeManager) {
                            try {
                                ...
                                this.log('[DragonManager] Applied PetSizeManager sizing');
                            } catch (error) {
                                this.error('[DragonManager] Failed to apply PetSizeManager:', error);
                            }
                        }
                        ...
                        if (element.offsetWidth === 0 || element.offsetHeight === 0) {
                            ...
                            this.log('[DragonManager] Applied fallback size as last resort');
                        }
    ...
    /**
     * Actual method to apply dragon animation (separated to support delayed processing)
     * @param {HTMLElement} element - The NFT element to animate
     */
    doApplyDragonAnimation(element) {
        ...
        element.classList.add('dragon-nft');
        
        this.animatedDragons.set(element, {
            element,
            isFlipped,
        ...
        if (!success) {
            this.error('[DragonManager] Failed to prepare element for animation');
            return;
        }
        
        // start animation
        this.startDragonAnimation(element, isFlipped);
        
        // apply shadow - ensure shadow is applied before setting movement
        if (this.config.shadow.enabled) {
            this.applyDragonShadow(element);
        }
        ...
        this.updateDragonDirection(element, isFlipped);
        
        this.log(`[DragonManager] Dragon animation applied for: ${element.dataset.name || 'unnamed dragon'}`);
    }
    
    applyDragonShadow(element) {
        if (!this.config.shadow.enabled || !window.PetShadowManager) return;
        
        // If already in tracking map, check if already has shadow
        const dragonData = this.animatedDragons.get(element);
        if (dragonData && dragonData.hasShadow) {
            this.log('[DragonManager] Dragon already has shadow, skipping:', element.dataset.name || element.dataset.id || 'unnamed');
            return;
        }
        
        // Add dragon-nft class to support shadow selector
        element.classList.add('dragon-nft');
        ...
        if (dragonData) {
            dragonData.hasShadow = true;
            this.animatedDragons.set(element, dragonData);
        }
        
        this.log('[DragonManager] Applied shadow to dragon:', element.dataset.name || element.dataset.id || 'unnamed');
    }
    
    /**
     * Enable movement for dragons
     * @param {HTMLElement} element - Dragon element
     */
    enableMovement(element) {
        if (!window.PetMovementManager) {
            this.log('[DragonManager] PetMovementManager not available, cannot enable movement');
            return;
        }
        ...
        const positionObserver = setInterval(() => {
            ...
            if (Math.abs(deltaX) > 0.5) {
                ...
                this.updateDragonDirection(element, isMovingRight);
                lastX = currentX;
            }
        }, 100); 
        
        const dragonData = this.animatedDragons.get(element) || {};
        dragonData.positionObserver = positionObserver;
        this.animatedDragons.set(element, dragonData);
        ...
        window.PetMovementManager.enableMovement(element, {
            ...
            onStateChange: (isMoving) => {
                ...
                const dragonEvent = new CustomEvent('dragon-moving-state-changed', {
                    detail: {
                        element: element,
                        isMoving: isMoving
                    },
        ...
        const directionHandler = (event) => {
            if (event.detail && event.detail.element === element) {
                ...
                this.updateDragonDirection(element, isRight);
            }
        };
        ...
        this.log(`[DragonManager] Enabled movement for dragon: ${element.dataset.name || 'unnamed'} with speed ${speed.toFixed(2)}`);
    }
    ...
    prepareElementForAnimation(element, isFlipped) {
        ...
        if (!element || !element.isConnected) {
            this.error('[DragonManager] Cannot prepare element - element is not connected to DOM');
            return false;
        }
        // Check element dimensions using multiple methods
        const offsetWidth = element.offsetWidth;
        ...
        this.log(`[DragonManager] Element dimension check: offset(${offsetWidth}x${offsetHeight}), css(${cssWidth}x${cssHeight})`);
        
        // Apply size using PetSizeManager first if available
        if (window.PetSizeManager) {
            window.PetSizeManager.setSize(element, 'dragon');
            this.log('[DragonManager] Applied size using PetSizeManager');
        }
        ...
        if (effectiveWidth === 0 || effectiveHeight === 0) {
            this.error('[DragonManager] Element still has zero effective dimensions after PetSizeManager, applying fallback size');
        ...
        try {
            ...
            const oldContainer = element.querySelector('.dragon-animation-container');
            if (oldContainer) {
            ...
            animContainer.className = 'dragon-animation-container';
            ...
            animContainer.style.zIndex = '10'; // Show above the dragon image
            ...
            if (shouldFlip) {
                this.log('[DragonManager] Setting initial animation direction: right (flipped)');
                ...
                this.log('[DragonManager] Setting initial animation direction: left (normal)');
            ...
            this.log(`[DragonManager] Element dimensions: ${elementWidth}x${elementHeight}`); 
            
            // Create canvas for sprite animation
            const canvas = document.createElement('canvas');
            canvas.className = 'dragon-animation-canvas';
            ...
            this.error('[DragonManager] Error preparing element for animation:', error);
            return false;
        }
    }
    
    /**
     * Start the dragon animation
     * @param {HTMLElement} element - The element to animate
     * @param {boolean} isFlipped - Whether the animation is flipped
     */
    startDragonAnimation(element, isFlipped) {
        // Check element
        if (!element || !element.isConnected) {
            this.error('[DragonManager] Cannot start animation - element is not connected to DOM');
            return;
        }
        
        try {
            // Get animation container
            const animContainer = element.querySelector('.dragon-animation-container');
            if (!animContainer) {
                this.error('[DragonManager] Animation container not found');
                return;
            }
            
            // Get the canvas
            const canvas = animContainer.querySelector('.dragon-animation-canvas');
            if (!canvas) {
                this.error('[DragonManager] Canvas not found');
                return;
            }
            
            const dragonData = this.animatedDragons.get(element);
            if (!dragonData) {
                this.error('[DragonManager] Dragon data not found');
                return;
            }
            
            // Wait until the atlas data is loaded
            if (!this.atlasData) {
                this.log('[DragonManager] Atlas data not loaded yet, creating fallback');
                this.createFallbackAtlasData();
            }
            ...
            this.error('[DragonManager] Error starting dragon animation:', error);
        }
    }
    ...
    startSpriteAnimation(element, canvas, isFlipped) {
        try {
            // Get dragon data from tracking map
            const dragonData = this.animatedDragons.get(element);
            ...
            if (!ctx) {
                this.error('[DragonManager] Failed to get canvas context');
                return;
            }
            
            // check if sprite sheets are loaded
            if (!this.loadedAnimations || Object.keys(this.loadedAnimations).length === 0) {
                this.log('[DragonManager] Sprite sheets not yet loaded, trying to load now...');
                
                // try to load main image from the path in config
                const leftImgPath = this.getAbsoluteResourcePath(this.config.animations.left);
                const img = new Image();
                img.onload = () => {
                    this.log('[DragonManager] Main image loaded for animation');
                ...
                img.onerror = (err) => {
                    this.error('[DragonManager] Failed to load image for animation:', err);
                };
                img.src = leftImgPath;
                return;
            }
            ...
            this.error('[DragonManager] Error in startSpriteAnimation:', error);
        }
    }
    ...
    initializeAnimation(element, canvas, ctx, isFlipped) {
        try {
            ...
            if (!element || !element.isConnected) {
                this.error('[DragonManager] Element no longer connected to DOM');
                return;
            }
            
            const dragonData = this.animatedDragons.get(element);
            if (!dragonData) {
                this.error('[DragonManager] Dragon data not found in initializeAnimation');
                return;
            }
            
            // check if sprite sheets are loaded
            if (!this.loadedAnimations || Object.keys(this.loadedAnimations).length === 0) {
                this.error('[DragonManager] Sprite sheets not loaded');
                return;
            }
            
            // at least one default image is required
            if (!this.loadedAnimations.default || !this.loadedAnimations.default.complete) {
                this.error('[DragonManager] Default sprite sheet not loaded or incomplete');
                return;
            }
            
            // Clear any existing animation
            if (dragonData.animationInterval) {
                clearInterval(dragonData.animationInterval);
                dragonData.animationInterval = null;
            }
            ...
            if (canvas.width !== elementWidth * 1.2 || canvas.height !== elementHeight * 1.2) {
                this.log(`[DragonManager] Updating canvas size to match element: ${elementWidth}x${elementHeight}`);
            ...
            const isMoving = element.classList.contains('pet-moving') || element.classList.contains('dragon-moving');
            
            dragonData.currentFrame = 0;
            dragonData.animationState = isMoving ? 'walking' : 'idle';
            
            // immediately draw the first frame
            this.drawFrame(ctx, canvas, this.loadedAnimations, dragonData.currentFrame);
            
            // set animation interval
            const frameDuration = this.config.animationSettings.frameDuration;
            
            dragonData.animationInterval = setInterval(() => {
                // check if element is still connected to DOM
                if (!element.isConnected) {
                    if (dragonData.animationInterval) {
                        clearInterval(dragonData.animationInterval);
                ...
                const isCurrentlyMoving = element.classList.contains('pet-moving') || 
                                          element.classList.contains('dragon-moving');
                
                // update animation based on current state
                if (isCurrentlyMoving) {
                    // when in moving state, play the complete walking animation
                    if (dragonData.animationState !== 'walking') {
                        // state changed from idle to moving, reset to the 0th frame
                        dragonData.currentFrame = 0;
                        dragonData.animationState = 'walking';
                    } else {
                        // loop within the walking frame range
                        const {start, end} = this.config.animationSettings.walkingFrames;
                        dragonData.currentFrame++;
                        
                        // loop back to the start frame
                        if (dragonData.currentFrame > end) {
                            dragonData.currentFrame = start;
                        }
                    }
                } else {
                    // when not in moving state, only display idle frame
                    if (dragonData.animationState !== 'idle') {
                        dragonData.currentFrame = this.config.animationSettings.idleFrames.start;
                        dragonData.animationState = 'idle';
                    }
                ...
                if (Math.abs(currentWidth - elementWidth) > elementWidth * 0.1 ||
                    ...
                    this.log(`[DragonManager] Element size changed, updated canvas: ${canvas.width}x${canvas.height}`);
                }
            ...
            this.error('[DragonManager] Error in initializeAnimation:', error);
        }
    }
    ...
    getCurrentRotationTestConfig() {
        ...
        if (now - this.rotationTestConfig.lastChangeTime > this.rotationTestConfig.timePerTest) {
            ...
            console.log(`[DragonManager] Test rotation configuration #${this.rotationTestConfig.currentTest}: ${currentConfig.description}`);
        }
    ...
    drawFrame(ctx, canvas, spriteSheets, frameIndex) {
        ...
        if (!frameData) {
            this.error(`[DragonManager] Frame data not found for: ${frameName}`);
            return;
        }
        ...
        try {
            ...
            this.error('[DragonManager] Error drawing frame:', error);
        }
    }
    
    pauseAllAnimations() {
        this.animatedDragons.forEach((dragonData, element) => {
            if (dragonData.animationInterval) {
                clearInterval(dragonData.animationInterval);
                dragonData.animationInterval = null;
            }
    ...
    resumeAllAnimations() {
        this.animatedDragons.forEach((dragonData, element) => {
            if (!dragonData.animationInterval && element.isConnected) {
                // Restart animation
                const animContainer = element.querySelector('.dragon-animation-container');
                if (animContainer) {
                    const canvas = animContainer.querySelector('.dragon-animation-canvas');
                    if (canvas) {
                        this.startDragonAnimation(element, dragonData.isFlipped);
                    }
                }
            }
        });
    ...
    cleanupNonExistentAnimations(currentNftIds) {
        ...
        elementsToRemove.forEach(element => {
            this.log(`[DragonManager] Cleaning up animation for removed element: ${element.dataset.id || element.dataset.name}`);
    ...
    updateDragonDirection(element, isRight) {
        this.log(`[DragonManager] Updating dragon direction: ${isRight ? 'right' : 'left'}`);
        ...
        const container = element.querySelector('.dragon-animation-container');
        if (container) {
            if (isRight) {
    ...
    cleanupAnimation(element) {
        ...
        if (dragonData.positionObserver) {
            clearInterval(dragonData.positionObserver);
        }
        ...
        this.animatedDragons.delete(element);
    }
    ...
    cleanupAllAnimations() {
        this.animatedDragons.forEach((_, element) => {
            this.cleanupAnimation(element);
        });
    }
     ...
     */
    resetProcessedState() {
        this.log('[DragonManager] Resetting processed state, will recheck all NFTs');
        this.processedNftIds.clear();
    ...
    setupDirectionChangeListener() {
        ...
        document.addEventListener('pet-direction-changed', (event) => {
            ...
            if (this.isDragonNFTElement(element)) {
                this.updateDragonDirection(element, direction === 'right');
            }
        });
        
        // if PetMovementManager is available, listen to its direction change event
        if (window.PetMovementManager) {
            this.log('[DragonManager] Adding PetMovementManager direction change hook');
            ...
            if (typeof originalUpdateDirectionState === 'function') {
                window.PetMovementManager.updateDirectionState = (pet) => {
                    ...
                    // then check if it is a dragon and update direction
                    if (this.isDragonNFTElement(pet.element)) {
                        // get direction information
                        const isFacingRight = pet.state.direction.x > 0;
                        
                        // delay update to ensure state has been updated by the original method
                        setTimeout(() => {
                            this.updateDragonDirection(pet.element, isFacingRight);
                        }, 10);
                    }
                };
            }
...
// Create and initialize the dragon manager
...
window.addEventListener('nftRefreshed', () => {
    if (dragonManager.initialized) {
        // When NFTs are refreshed, reset processed state and recheck
        dragonManager.resetProcessedState();
        // Delay slightly to ensure DOM has updated
        setTimeout(() => dragonManager.checkForDragons(), 500);
    }
});

// Export the dragon manager for use in other modules
window.DragonManager = dragonManager;

// Add the backward compatibility method checkForDragons just like in egg.js
// First check if this is already using PetAnimationBase
if (DragonManager.prototype instanceof PetAnimationBase) {
    /**
     * Backward compatibility method - calls checkForPets
     * @param {boolean} applyShadows - Whether to apply shadows
     */
    DragonManager.prototype.checkForDragons = function(applyShadows = true) {
