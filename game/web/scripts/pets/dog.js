// DogManager class to handle all dog-related functionality
class DogManager {
    constructor() {
        ...
        this.config = {
            ...
            shadow: {
                ...
                applyToExisting: true    // Whether to apply to existing dogs
            }
        };
    ...
    /**
     * Initialize the Dog Manager
     */
    async init() {
        if (this.initialized) return;
        
        this.log('[DogManager] Initializing dog animation system...');
        
        // Load and parse the atlas file
        try {
            await this.loadAtlasData();
        } catch (error) {
            this.error('[DogManager] Failed to load atlas data:', error);
        }
        
        // Listen for NFT loaded events to detect dogs
        ...
        // Check for dogs immediately if we're on the main page
        if (window.location.href.includes('home.html')) {
            // Short delay to ensure DOM is fully loaded
            setTimeout(() => this.checkForDogs(), 1000);
        }
        
        // Setup periodic dog checking on main page
        this.setupPeriodicDogCheck();
    ...
    initShadowSystem() {
        ...
        this.log('[DogManager] Initializing shadow system...');
        
        // Check if PetShadowManager is available
        if (window.PetShadowManager) {
            this.log('[DogManager] PetShadowManager found, will use for dog shadows');
            
            if (this.config.shadow.applyToExisting) {
                setTimeout(() => {
                    this.applyAllDogShadows();
                }, 1500);
            }
            
            document.addEventListener('dog-moving-state-changed', (event) => {
                if (event.detail && event.detail.element) {
                    const dogElement = event.detail.element;
                    
                    // Update shadow state
                    if (window.PetShadowManager && window.PetShadowManager.updateShadow) {
                        window.PetShadowManager.updateShadow(dogElement);
                    }
                }
            });
        } else {
            this.log('[DogManager] PetShadowManager not available, shadows will not be applied');
        }
    }
    ...
    applyAllDogShadows() {
        ...
        this.log('[DogManager] Applying shadows to all dogs...');
        
        const dogElements = document.querySelectorAll('.dog-nft');
        let count = 0;
        
        dogElements.forEach(dogElement => {
            // Add shadow
            window.PetShadowManager.addShadow(dogElement, {
                ratio: 0.8,  // Shadow width relative to dog
                jumpEffect: true  // Enable jumping effect
            });
            count++;
        });
        
        this.log(`[DogManager] Applied shadows to ${count} dogs`);
    }
    ...
    async loadAtlasData() {
        ...
        try {
            ...
            this.log(`[DogManager] Atlas path: ${atlasPath}`);
            ...
            this.log('[DogManager] Atlas data parsed using SpriteAtlasHelper');
            
            // get all image files to load
            const imageFiles = this.atlasData.meta.images.map(img => img.file);
            this.log(`[DogManager] Atlas references ${imageFiles.length} image files:`, imageFiles);
            ...
            this.log('[DogManager] Atlas data and all images loaded successfully');
        } catch (error) {
            this.error('[DogManager] Error loading atlas file:', error);
    ...
    async preloadAllAnimations(basePath, imageFiles) {
        try {
            ...
            this.log(`[DogManager] Successfully loaded ${Object.keys(this.loadedAnimations).length - 1} sprite sheets`);
        } catch (error) {
            this.error('[DogManager] Error preloading animations:', error);
    ...
    preloadAnimations() {
        ...
        if (!this.loadedAnimations || Object.keys(this.loadedAnimations).length === 0) {
            ...
            leftImg.onload = () => {
                this.log(`[DogManager] Main image preloaded successfully`);
                this.loadedAnimations = { default: leftImg, 0: leftImg };
            };
            leftImg.onerror = (err) => {
                this.error(`[DogManager] Error preloading main image:`, err);
            };
            
            // Get full path
            const imagePath = this.getAbsoluteResourcePath(this.config.animations.left);
            this.log(`[DogManager] Main image path: ${imagePath}`);
    ...
    setupPeriodicDogCheck() {
        if (window.location.href.includes('home.html')) {
            setInterval(() => {
                this.checkForDogs(false); // Pass false to indicate not to auto-apply shadows
            }, 10000);
        }
    }
    ...
    /**
     * Observe the NFT container for new dogs
     */
    observeNFTContainer() {
        ...
        const observer = new MutationObserver((mutations) => {
            let shouldCheckDogs = false;
            
            mutations.forEach(mutation => {
                if (mutation.type === 'childList' && mutation.addedNodes.length > 0) {
                    shouldCheckDogs = true;
                }
            });
        ...
        this.log('[DogManager] Now monitoring farm container for new NFTs');
    }
    ...
    createFallbackAtlasData() {
        this.log('[DogManager] Creating fallback atlas data');
    ...
    handleNFTLoadedEvent(event) {
        ...
        if (!nftElement.isConnected) {
            this.log(`[DogManager] NFT element not connected to DOM, skipping processing`);
            return;
        }
        ...
        if (this.processedNftIds.has(nftId)) {
            this.log(`[DogManager] NFT already processed, skipping: ${nftId}`);
            return;
        }
        
        // If element already has animation, skip
        if (this.animatedDogs.has(nftElement)) {
            this.log(`[DogManager] Element already animated, skipping: ${nftId}`);
            return;
        }
        
        // Check if this is a dog
        if (this.isDogNFT(nftData)) {
            this.log('[DogManager] Dog NFT detected from event:', nftData.metadata?.name || nftData.name || nftId);
            
            // Mark this NFT as processed
            this.processedNftIds.add(nftId);
            
            // Apply animation
            this.applyDogAnimation(nftElement);
        }
    }
    
    /**
     * Check the page for dog NFTs and apply animations
     * @param {boolean} applyShadows - Whether to apply shadows, default is true
     */
    checkForDogs(applyShadows = true) {
        this.log('[DogManager] Checking for dogs on the page...');
        
        // Find all NFT elements in the farm
        const farmContainer = document.getElementById('farm-animals-container');
        if (!farmContainer) {
            this.error('[DogManager] Farm container not found');
            return;
        }
        
        // Find all animal elements
        const animalElements = farmContainer.querySelectorAll('.animal');
        this.log(`[DogManager] Found ${animalElements.length} animal elements`);
        ...
        animalElements.forEach((animalElement, index) => {
            ...
            if (this.animatedDogs.has(animalElement)) {
                this.log(`[DogManager] Element already animated, skipping: ${nftId}`);
                return;
            }
            
            // Check if this is a dog
            if (this.isDogNFTElement(animalElement)) {
                this.log(`[DogManager] Found dog NFT #${index} (ID: ${nftId}):`, animalElement.dataset.name);
                
                // Mark this NFT as processed
                this.processedNftIds.add(nftId);
                
                // Apply animation
                this.applyDogAnimation(animalElement);
            }
        });
    ...
    isDogNFTElement(element) {
        // Check if the element has dataset
        if (!element.dataset) return false;
        
        // Check for dog name, type, or class
        const name = element.dataset.name || '';
        const type = element.dataset.type || '';
        
        const hasDogName = name.toLowerCase().includes('dog');
        const hasDogType = type.toLowerCase() === 'dog';
        const hasDogClass = element.classList.contains('dog-nft') || element.classList.contains('dog');
        
        return hasDogName || hasDogType || hasDogClass;
    }
    ...
    isDogNFT(nftData) {
        ...
        if (nftData.metadata && nftData.metadata.name) {
            return nftData.metadata.name.toLowerCase().includes('dog');
        }
        
        // Check direct name property
        if (nftData.name) {
            return nftData.name.toLowerCase().includes('dog');
        }
        
        // Check any other properties that could identify a dog
        if (nftData.type) {
            return nftData.type.toLowerCase() === 'dog';
        }
        
        return false;
    }

    applyDogAnimation(element) {
        if (this.animatedDogs.has(element)) return;
        
        this.log('[DogManager] Applying dog animation to element:', element);
        
        // Check if element is visible and has dimensions
        if (!element.isConnected) {
            this.error('[DogManager] Element is not connected to DOM, cannot animate:', element);
            return;
        }
        ...
        if (window.PetSizeHelper) {
            window.PetSizeHelper.applyAnimationWithSizeCheck(
                element,
                'dog',
                (el) => this.doApplyDogAnimation(el),
                {
                    maxWaitAttempts: 8,
            ...
            this.log('[DogManager] PetSizeHelper not available, using fallback logic');
            ...
            if (effectiveWidth === 0 || effectiveHeight === 0) {
                this.log('[DogManager] Element has zero effective dimensions, delaying animation:', element);
                ...
                setTimeout(() => {
                    ...
                    if (element.isConnected && (newEffectiveWidth > 0 && newEffectiveHeight > 0)) {
                        this.log('[DogManager] Element now has dimensions, applying animation');
                        this.doApplyDogAnimation(element);
                    } else {
                        this.error('[DogManager] Element still has zero dimensions after delay, using default size');
                        ...
                        if (window.PetSizeManager) {
                            try {
                                window.PetSizeManager.setSize(element, 'dog');
                                this.log('[DogManager] Re-applied PetSizeManager after fallback sizing');
                            } catch (error) {
                                this.error('[DogManager] Failed to re-apply PetSizeManager:', error);
                            }
                        }
    ...
    /**
     * Actual method to apply dog animation (separated to support delayed processing)
     * @param {HTMLElement} element - The NFT element to animate
     */
    doApplyDogAnimation(element) {
        ...
        element.classList.add('dog-nft');
        
        this.animatedDogs.set(element, {
            element,
            isFlipped,
        ...
        if (!success) {
            this.error('[DogManager] Failed to prepare element for animation');
            return;
        }
        
        // start animation
        this.startDogAnimation(element, isFlipped);
        
        // apply shadow - ensure shadow is applied before setting movement
        if (this.config.shadow.enabled) {
            this.applyDogShadow(element);
        }
        ...
        this.updateDogDirection(element, isFlipped);
        
        this.log(`[DogManager] Dog animation applied for: ${element.dataset.name || 'unnamed dog'}`);
    }
    
    applyDogShadow(element) {
        if (!this.config.shadow.enabled || !window.PetShadowManager) return;
        
        // If already in tracking map, check if already has shadow
        const dogData = this.animatedDogs.get(element);
        if (dogData && dogData.hasShadow) {
            this.log('[DogManager] Dog already has shadow, skipping:', element.dataset.name || element.dataset.id || 'unnamed');
            return;
        }
        
        // Add dog-nft class to support shadow selector
        element.classList.add('dog-nft');
        ...
        if (dogData) {
            dogData.hasShadow = true;
            this.animatedDogs.set(element, dogData);
        }
        
        this.log('[DogManager] Applied shadow to dog:', element.dataset.name || element.dataset.id || 'unnamed');
    }
    
    /**
     * Enable movement for dogs
     * @param {HTMLElement} element - Dog element
     */
    enableMovement(element) {
        if (!window.PetMovementManager) {
            this.log('[DogManager] PetMovementManager not available, cannot enable movement');
            return;
        }
        ...
        const positionObserver = setInterval(() => {
            ...
            if (Math.abs(deltaX) > 0.5) {
                ...
                this.updateDogDirection(element, isMovingRight);
                lastX = currentX;
            }
        }, 100); 
        
        const dogData = this.animatedDogs.get(element) || {};
        dogData.positionObserver = positionObserver;
        this.animatedDogs.set(element, dogData);
        ...
        window.PetMovementManager.enableMovement(element, {
            ...
            onStateChange: (isMoving) => {
                ...
                const dogEvent = new CustomEvent('dog-moving-state-changed', {
                    detail: {
                        element: element,
                        isMoving: isMoving
                    },
        ...
        const directionHandler = (event) => {
            if (event.detail && event.detail.element === element) {
                ...
                this.updateDogDirection(element, isRight);
            }
        };
        ...
        this.log(`[DogManager] Enabled movement for dog: ${element.dataset.name || 'unnamed'} with speed ${speed.toFixed(2)}`);
    }
    ...
    prepareElementForAnimation(element, isFlipped) {
        ...
        if (!element || !element.isConnected) {
            this.error('[DogManager] Cannot prepare element - element is not connected to DOM');
            return false;
        }
        // Check element dimensions using multiple methods
        const offsetWidth = element.offsetWidth;
        ...
        this.log(`[DogManager] Element dimension check: offset(${offsetWidth}x${offsetHeight}), css(${cssWidth}x${cssHeight})`);
        
        // Apply size using PetSizeManager first if available
        if (window.PetSizeManager) {
            window.PetSizeManager.setSize(element, 'dog');
            this.log('[DogManager] Applied size using PetSizeManager');
        }
        ...
        if (effectiveWidth === 0 || effectiveHeight === 0) {
            this.error('[DogManager] Element still has zero effective dimensions after PetSizeManager, applying fallback size');
            // Force set minimum size with important flag
            element.style.setProperty('width', '50px', 'important');
            element.style.setProperty('height', '50px', 'important');
            element.style.setProperty('min-width', '50px', 'important');
        ...
        try {
            ...
            const oldContainer = element.querySelector('.dog-animation-container');
            if (oldContainer) {
            ...
            animContainer.className = 'dog-animation-container';
            ...
            animContainer.style.zIndex = '10'; // Show above the dog image
            ...
            if (shouldFlip) {
                this.log('[DogManager] Setting initial animation direction: right (flipped)');
                ...
                this.log('[DogManager] Setting initial animation direction: left (normal)');
            ...
            this.log(`[DogManager] Element dimensions: ${elementWidth}x${elementHeight}`); 
            
            // Create canvas for sprite animation
            const canvas = document.createElement('canvas');
            canvas.className = 'dog-animation-canvas';
            ...
            this.error('[DogManager] Error preparing element for animation:', error);
            return false;
        }
    }
     ...
     */
    startDogAnimation(element, isFlipped) {
        // Check element
        if (!element || !element.isConnected) {
            this.error('[DogManager] Cannot start animation - element is not connected to DOM');
            return;
        }
        
        try {
            // Get animation container
            const animContainer = element.querySelector('.dog-animation-container');
            if (!animContainer) {
                this.error('[DogManager] Animation container not found');
                return;
            }
            
            // Get the canvas
            const canvas = animContainer.querySelector('.dog-animation-canvas');
            if (!canvas) {
                this.error('[DogManager] Canvas not found');
                return;
            }
            
            const dogData = this.animatedDogs.get(element);
            if (!dogData) {
                this.error('[DogManager] Dog data not found');
                return;
            }
            
            // Wait until the atlas data is loaded
            if (!this.atlasData) {
                this.log('[DogManager] Atlas data not loaded yet, creating fallback');
                this.createFallbackAtlasData();
            }
            ...
            this.error('[DogManager] Error starting dog animation:', error);
        }
    }
    ...
    startSpriteAnimation(element, canvas, isFlipped) {
        try {
            // Get dog data from tracking map
            const dogData = this.animatedDogs.get(element);
            if (!dogData) return;
            ...
            if (!ctx) {
                this.error('[DogManager] Failed to get canvas context');
                return;
            }
            
            // check if sprite sheets are loaded
            if (!this.loadedAnimations || Object.keys(this.loadedAnimations).length === 0) {
                this.log('[DogManager] Sprite sheets not yet loaded, trying to load now...');
                
                // try to load main image from the path in config
                const leftImgPath = this.getAbsoluteResourcePath(this.config.animations.left);
                const img = new Image();
                img.onload = () => {
                    this.log('[DogManager] Main image loaded for animation');
                ...
                img.onerror = (err) => {
                    this.error('[DogManager] Failed to load image for animation:', err);
                };
                img.src = leftImgPath;
                return;
            }
            ...
            this.error('[DogManager] Error in startSpriteAnimation:', error);
        }
    }
    ...
    initializeAnimation(element, canvas, ctx, isFlipped) {
        try {
            ...
            if (!element || !element.isConnected) {
                this.error('[DogManager] Element no longer connected to DOM');
                return;
            }
            
            const dogData = this.animatedDogs.get(element);
            if (!dogData) {
                this.error('[DogManager] Dog data not found in initializeAnimation');
                return;
            }
            
            // check if sprite sheets are loaded
            if (!this.loadedAnimations || Object.keys(this.loadedAnimations).length === 0) {
                this.error('[DogManager] Sprite sheets not loaded');
                return;
            }
            
            // at least one default image is required
            if (!this.loadedAnimations.default || !this.loadedAnimations.default.complete) {
                this.error('[DogManager] Default sprite sheet not loaded or incomplete');
                return;
            }
            
            // Clear any existing animation
            if (dogData.animationInterval) {
                clearInterval(dogData.animationInterval);
                dogData.animationInterval = null;
            }
            ...
            if (canvas.width !== elementWidth * 1.2 || canvas.height !== elementHeight * 1.2) {
                this.log(`[DogManager] Updating canvas size to match element: ${elementWidth}x${elementHeight}`);
            ...
            const isMoving = element.classList.contains('pet-moving') || element.classList.contains('dog-moving');
            
            dogData.currentFrame = 0;
            dogData.animationState = isMoving ? 'walking' : 'idle';
            
            // immediately draw the first frame
            this.drawFrame(ctx, canvas, this.loadedAnimations, dogData.currentFrame);
            
            // set animation interval
            const frameDuration = this.config.animationSettings.frameDuration;
            
            dogData.animationInterval = setInterval(() => {
                // check if element is still connected to DOM
                if (!element.isConnected) {
                    if (dogData.animationInterval) {
                        clearInterval(dogData.animationInterval);
                ...
                const isCurrentlyMoving = element.classList.contains('pet-moving') || 
                                          element.classList.contains('dog-moving');
                
                // update animation based on current state
                if (isCurrentlyMoving) {
                    // when in moving state, play the complete walking animation
                    if (dogData.animationState !== 'walking') {
                        // state changed from idle to moving, reset to the 0th frame
                        dogData.currentFrame = 0;
                        dogData.animationState = 'walking';
                    } else {
                        // loop within the walking frame range
                        const {start, end} = this.config.animationSettings.walkingFrames;
                        dogData.currentFrame++;
                        
                        // loop back to the start frame
                        if (dogData.currentFrame > end) {
                            dogData.currentFrame = start;
                        }
                    }
                } else {
                    // when not in moving state, only display idle frame
                    if (dogData.animationState !== 'idle') {
                        dogData.currentFrame = this.config.animationSettings.idleFrames.start;
                        dogData.animationState = 'idle';
                    }
                ...
                if (Math.abs(currentWidth - elementWidth) > elementWidth * 0.1 ||
                    ...
                    this.log(`[DogManager] Element size changed, updated canvas: ${canvas.width}x${canvas.height}`);
                }
            ...
            this.error('[DogManager] Error in initializeAnimation:', error);
        }
    }
    ...
    getCurrentRotationTestConfig() {
        ...
        if (now - this.rotationTestConfig.lastChangeTime > this.rotationTestConfig.timePerTest) {
            ...
            console.log(`[DogManager] Test rotation configuration #${this.rotationTestConfig.currentTest}: ${currentConfig.description}`);
        }
    ...
    drawFrame(ctx, canvas, spriteSheets, frameIndex) {
        ...
        if (!frameData) {
            this.error(`[DogManager] Frame data not found for: ${frameName}`);
            return;
        }
        ...
        try {
            ...
            this.error('[DogManager] Error drawing frame:', error);
        }
    }
    
    pauseAllAnimations() {
        this.animatedDogs.forEach((dogData, element) => {
            if (dogData.animationInterval) {
                clearInterval(dogData.animationInterval);
                dogData.animationInterval = null;
            }
    ...
    resumeAllAnimations() {
        this.animatedDogs.forEach((dogData, element) => {
            if (!dogData.animationInterval && element.isConnected) {
                // Restart animation
                const animContainer = element.querySelector('.dog-animation-container');
                if (animContainer) {
                    const canvas = animContainer.querySelector('.dog-animation-canvas');
                    if (canvas) {
                        this.startDogAnimation(element, dogData.isFlipped);
                    }
                }
            }
        });
    ...
    cleanupNonExistentAnimations(currentNftIds) {
        ...
        elementsToRemove.forEach(element => {
            this.log(`[DogManager] Cleaning up animation for removed element: ${element.dataset.id || element.dataset.name}`);
    ...
    updateDogDirection(element, isRight) {
        this.log(`[DogManager] Updating dog direction: ${isRight ? 'right' : 'left'}`);
        ...
        const container = element.querySelector('.dog-animation-container');
        if (container) {
            if (isRight) {
    ...
    cleanupAnimation(element) {
        ...
        if (dogData.positionObserver) {
            clearInterval(dogData.positionObserver);
        }
        ...
        this.animatedDogs.delete(element);
    }
    ...
    cleanupAllAnimations() {
        this.animatedDogs.forEach((_, element) => {
            this.cleanupAnimation(element);
        });
    }
     ...
     */
    resetProcessedState() {
        this.log('[DogManager] Resetting processed state, will recheck all NFTs');
        this.processedNftIds.clear();
    ...
    setupDirectionChangeListener() {
        ...
        document.addEventListener('pet-direction-changed', (event) => {
            ...
            if (this.isDogNFTElement(element)) {
                this.updateDogDirection(element, direction === 'right');
            }
        });
        
        // if PetMovementManager is available, listen to its direction change event
        if (window.PetMovementManager) {
            this.log('[DogManager] Adding PetMovementManager direction change hook');
            ...
            if (typeof originalUpdateDirectionState === 'function') {
                window.PetMovementManager.updateDirectionState = (pet) => {
                    ...
                    // then check if it is a dog and update direction
                    if (this.isDogNFTElement(pet.element)) {
                        // get direction information
                        const isFacingRight = pet.state.direction.x > 0;
                        
                        // delay update to ensure state has been updated by the original method
                        setTimeout(() => {
                            this.updateDogDirection(pet.element, isFacingRight);
                        }, 10);
                    }
                };
            }
...
// Create and initialize the dog manager
...
window.addEventListener('nftRefreshed', () => {
    if (dogManager.initialized) {
        // When NFTs are refreshed, reset processed state and recheck
        dogManager.resetProcessedState();
        // Delay slightly to ensure DOM has updated
        setTimeout(() => dogManager.checkForDogs(), 500);
    }
});

// Export the dog manager for use in other modules
window.DogManager = dogManager;

// Add the backward compatibility method checkForDogs just like in egg.js
// First check if this is already using PetAnimationBase
if (DogManager.prototype instanceof PetAnimationBase) {
    /**
     * Backward compatibility method - calls checkForPets
     * @param {boolean} applyShadows - Whether to apply shadows
     */
    DogManager.prototype.checkForDogs = function(applyShadows = true) {
