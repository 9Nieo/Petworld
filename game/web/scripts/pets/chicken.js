...
class ChickenManager {
    constructor() {
        ...
        this.config = {
            ...
            animations: {
                left: '../../resources/actions/Chicken/left.png',
                leftAtlas: '../../resources/actions/Chicken/left.atlas'
            },
            // Animation settings
            animationSettings: {
            ...
            shadow: {
                ...
                applyToExisting: true    // Whether to apply to existing chickens
            }
        };
        ...
        // Track animated chickens
        this.animatedChickens = new Map();
    ...
    /**
     * Initialize the Chicken Manager
     */
    async init() {
        if (this.initialized) return;
        
        this.log('[ChickenManager] Initializing chicken animation system...');
        
        // Load and parse the atlas file
        try {
            await this.loadAtlasData();
        } catch (error) {
            this.error('[ChickenManager] Failed to load atlas data:', error);
        }
        
        // Listen for NFT loaded events to detect chickens
        ...
        // Check for chickens immediately if we're on the main page
        if (window.location.href.includes('home.html')) {
            // Short delay to ensure DOM is fully loaded
            setTimeout(() => this.checkForChickens(), 1000);
        }
        
        // Setup periodic chicken checking on main page
        this.setupPeriodicChickenCheck();
    ...
    initShadowSystem() {
        ...
        this.log('[ChickenManager] Initializing shadow system...');
        
        // Check if PetShadowManager is available
        if (window.PetShadowManager) {
            this.log('[ChickenManager] PetShadowManager found, will use for chicken shadows');
            
            if (this.config.shadow.applyToExisting) {
                setTimeout(() => {
                    this.applyAllChickenShadows();
                }, 1500);
            }
            
            document.addEventListener('chicken-moving-state-changed', (event) => {
                if (event.detail && event.detail.element) {
                    const chickenElement = event.detail.element;
                    
                    // Update shadow state
                    if (window.PetShadowManager && window.PetShadowManager.updateShadow) {
                        window.PetShadowManager.updateShadow(chickenElement);
                    }
                }
            });
        } else {
            this.log('[ChickenManager] PetShadowManager not available, shadows will not be applied');
        }
    }
    
    applyAllChickenShadows() {
        if (!window.PetShadowManager) return;
        
        this.log('[ChickenManager] Applying shadows to all chickens...');
        
        const chickenElements = document.querySelectorAll('.chicken-nft');
        let count = 0;
        
        chickenElements.forEach(chickenElement => {
            // Add shadow
            window.PetShadowManager.addShadow(chickenElement, {
                ratio: 0.8,  // Shadow width relative to chicken
                jumpEffect: true  // Enable jumping effect
            });
            count++;
        });
        
        this.log(`[ChickenManager] Applied shadows to ${count} chickens`);
    }
    ...
    async loadAtlasData() {
        ...
        try {
            ...
            this.log(`[ChickenManager] Atlas path: ${atlasPath}`);
            ...
            this.log('[ChickenManager] Atlas data parsed using SpriteAtlasHelper');
            
            // get all image files to load
            const imageFiles = this.atlasData.meta.images.map(img => img.file);
            this.log(`[ChickenManager] Atlas references ${imageFiles.length} image files:`, imageFiles);
            ...
            this.log('[ChickenManager] Atlas data and all images loaded successfully');
        } catch (error) {
            this.error('[ChickenManager] Error loading atlas file:', error);
    ...
    async preloadAllAnimations(basePath, imageFiles) {
        try {
            ...
            this.log(`[ChickenManager] Successfully loaded ${Object.keys(this.loadedAnimations).length - 1} sprite sheets`);
        } catch (error) {
            this.error('[ChickenManager] Error preloading animations:', error);
    ...
    preloadAnimations() {
        ...
        if (!this.loadedAnimations || Object.keys(this.loadedAnimations).length === 0) {
            ...
            leftImg.onload = () => {
                this.log(`[ChickenManager] Main image preloaded successfully`);
                this.loadedAnimations = { default: leftImg, 0: leftImg };
            };
            leftImg.onerror = (err) => {
                this.error(`[ChickenManager] Error preloading main image:`, err);
            };
            
            // Get full path
            const imagePath = this.getAbsoluteResourcePath(this.config.animations.left);
            this.log(`[ChickenManager] Main image path: ${imagePath}`);
    ...
    /**
     * Observe the NFT container for new chickens
     */
    observeNFTContainer() {
        ...
        const observer = new MutationObserver((mutations) => {
            let shouldCheckChickens = false;
            
            mutations.forEach(mutation => {
                if (mutation.type === 'childList' && mutation.addedNodes.length > 0) {
                    shouldCheckChickens = true;
                }
            });
            
            if (shouldCheckChickens) {
                this.checkForChickens();
            }
        });
        ...
        this.log('[ChickenManager] Now monitoring farm container for new NFTs');
    }
    ...
    createFallbackAtlasData() {
        this.log('[ChickenManager] Creating fallback atlas data');
    ...
    handleNFTLoadedEvent(event) {
        ...
        if (!nftElement.isConnected) {
            this.log(`[ChickenManager] NFT element not connected to DOM, skipping processing`);
            return;
        }
        ...
        if (this.processedNftIds.has(nftId)) {
            this.log(`[ChickenManager] NFT already processed, skipping: ${nftId}`);
            return;
        }
        
        // If element already has animation, skip
        if (this.animatedChickens.has(nftElement)) {
            this.log(`[ChickenManager] Element already animated, skipping: ${nftId}`);
            return;
        }
        
        // Check if this is a chicken
        if (this.isChickenNFT(nftData)) {
            this.log('[ChickenManager] Chicken NFT detected from event:', nftData.metadata?.name || nftData.name || nftId);
            
            // Mark this NFT as processed
            this.processedNftIds.add(nftId);
            
            // Apply animation
            this.applyChickenAnimation(nftElement);
        }
    }
    
    /**
     * Check the page for chicken NFTs and apply animations
     * @param {boolean} applyShadows - Whether to apply shadows, default is true
     */
    checkForChickens(applyShadows = true) {
        this.log('[ChickenManager] Checking for chickens on the page...');
        
        // Find all NFT elements in the farm
        const farmContainer = document.getElementById('farm-animals-container');
        if (!farmContainer) {
            this.error('[ChickenManager] Farm container not found');
            return;
        }
        
        // Find all animal elements
        const animalElements = farmContainer.querySelectorAll('.animal');
        this.log(`[ChickenManager] Found ${animalElements.length} animal elements`);
        ...
        animalElements.forEach((animalElement, index) => {
            ...
            if (this.animatedChickens.has(animalElement)) {
                this.log(`[ChickenManager] Element already animated, skipping: ${nftId}`);
                return;
            }
            
            // Check if this is a chicken
            if (this.isChickenNFTElement(animalElement)) {
                this.log(`[ChickenManager] Found chicken NFT #${index} (ID: ${nftId}):`, animalElement.dataset.name);
                
                // Mark this NFT as processed
                this.processedNftIds.add(nftId);
                
                // Apply animation
                this.applyChickenAnimation(animalElement);
            }
        });
    ...
    isChickenNFTElement(element) {
        // Check if the element has dataset
        if (!element.dataset) return false;
        
        // Check for chicken name, type, or class
        const name = element.dataset.name || '';
        const type = element.dataset.type || '';
        
        const hasChickenName = name.toLowerCase().includes('chicken');
        const hasChickenType = type.toLowerCase() === 'chicken';
        const hasChickenClass = element.classList.contains('chicken-nft') || element.classList.contains('chicken');
        
        return hasChickenName || hasChickenType || hasChickenClass;
    }
    
    isChickenNFT(nftData) {
        if (!nftData) return false;
        
        // Check metadata name
        if (nftData.metadata && nftData.metadata.name) {
            return nftData.metadata.name.toLowerCase().includes('chicken');
        }
        
        // Check direct name property
        if (nftData.name) {
            return nftData.name.toLowerCase().includes('chicken');
        }
        
        // Check any other properties that could identify a chicken
        if (nftData.type) {
            return nftData.type.toLowerCase() === 'chicken';
        }
        
        return false;
    }

    applyChickenAnimation(element) {
        if (this.animatedChickens.has(element)) return;
        
        this.log('[ChickenManager] Applying chicken animation to element:', element);
    ...
    /**
     * Actual method to apply chicken animation (separated to support delayed processing)
     * @param {HTMLElement} element - The NFT element to animate
     */
    doApplyChickenAnimation(element) {
        ...
        element.classList.add('chicken-nft');
        
        this.animatedChickens.set(element, {
            element,
            isFlipped,
        ...
        if (!success) {
            this.error('[ChickenManager] Failed to prepare element for animation');
            return;
        }
        
        // start animation
        this.startChickenAnimation(element, isFlipped);
        
        // apply shadow - ensure shadow is applied before setting movement
        if (this.config.shadow.enabled) {
            this.applyChickenShadow(element);
        }
        ...
        this.updateChickenDirection(element, isFlipped);
        
        this.log(`[ChickenManager] Chicken animation applied for: ${element.dataset.name || 'unnamed chicken'}`);
    }
    
    applyChickenShadow(element) {
        if (!this.config.shadow.enabled || !window.PetShadowManager) return;
        
        // If already in tracking map, check if already has shadow
        const chickenData = this.animatedChickens.get(element);
        if (chickenData && chickenData.hasShadow) {
            this.log('[ChickenManager] Chicken already has shadow, skipping:', element.dataset.name || element.dataset.id || 'unnamed');
            return;
        }
        
        // Add chicken-nft class to support shadow selector
        element.classList.add('chicken-nft');
        ...
        if (chickenData) {
            chickenData.hasShadow = true;
            this.animatedChickens.set(element, chickenData);
        }
        
        this.log('[ChickenManager] Applied shadow to chicken:', element.dataset.name || element.dataset.id || 'unnamed');
    }
    
    /**
     * Enable movement for chickens
     * @param {HTMLElement} element - Chicken element
     */
    enableMovement(element) {
        if (!window.PetMovementManager) {
            this.log('[ChickenManager] PetMovementManager not available, cannot enable movement');
            return;
        }
        ...
        const positionObserver = setInterval(() => {
            ...
            if (Math.abs(deltaX) > 0.5) {
                ...
                this.updateChickenDirection(element, isMovingRight);
                lastX = currentX;
            }
        }, 100); 
        
        const chickenData = this.animatedChickens.get(element) || {};
        chickenData.positionObserver = positionObserver;
        this.animatedChickens.set(element, chickenData);
        ...
        window.PetMovementManager.enableMovement(element, {
            ...
            onStateChange: (isMoving) => {
                ...
                const chickenEvent = new CustomEvent('chicken-moving-state-changed', {
                    detail: {
                        element: element,
                        isMoving: isMoving
                    },
        ...
        const directionHandler = (event) => {
            if (event.detail && event.detail.element === element) {
                ...
                this.updateChickenDirection(element, isRight);
            }
        };
        ...
        this.log(`[ChickenManager] Enabled movement for chicken: ${element.dataset.name || 'unnamed'} with speed ${speed.toFixed(2)}`);
    }
    ...
    prepareElementForAnimation(element, isFlipped) {
        ...
        try {
            ...
            const oldContainer = element.querySelector('.chicken-animation-container');
            if (oldContainer) {
            ...
            animContainer.className = 'chicken-animation-container';
            ...
            animContainer.style.zIndex = '10'; // Show above the chicken image
            ...
            if (shouldFlip) {
                this.log('[ChickenManager] Setting initial animation direction: right (flipped)');
                ...
                this.log('[ChickenManager] Setting initial animation direction: left (normal)');
            ...
            this.log(`[ChickenManager] Element dimensions: ${elementWidth}x${elementHeight}`); 
            
            // Create canvas for sprite animation
            const canvas = document.createElement('canvas');
            canvas.className = 'chicken-animation-canvas';
            ...
            this.error('[ChickenManager] Error preparing element for animation:', error);
            return false;
        }
    }
     ...
     */
    startChickenAnimation(element, isFlipped) {
        // Check element
        if (!element || !element.isConnected) {
            this.error('[ChickenManager] Cannot start animation - element is not connected to DOM');
            return;
        }
        
        try {
            // Get animation container
            const animContainer = element.querySelector('.chicken-animation-container');
            if (!animContainer) {
                this.error('[ChickenManager] Animation container not found');
                return;
            }
            
            // Get the canvas
            const canvas = animContainer.querySelector('.chicken-animation-canvas');
            if (!canvas) {
                this.error('[ChickenManager] Canvas not found');
                return;
            }
            
            const chickenData = this.animatedChickens.get(element);
            if (!chickenData) {
                this.error('[ChickenManager] Chicken data not found');
                return;
            }
            
            // Wait until the atlas data is loaded
            if (!this.atlasData) {
                this.log('[ChickenManager] Atlas data not loaded yet, creating fallback');
                this.createFallbackAtlasData();
            }
            ...
            this.error('[ChickenManager] Error starting chicken animation:', error);
        }
    }
    ...
    startSpriteAnimation(element, canvas, isFlipped) {
        try {
            // Get chicken data from tracking map
            const chickenData = this.animatedChickens.get(element);
            ...
            if (!ctx) {
                this.error('[ChickenManager] Failed to get canvas context');
                return;
            }
            
            // check if sprite sheets are loaded
            if (!this.loadedAnimations || Object.keys(this.loadedAnimations).length === 0) {
                this.log('[ChickenManager] Sprite sheets not yet loaded, trying to load now...');
                
                // try to load main image from the path in config
                const leftImgPath = this.getAbsoluteResourcePath(this.config.animations.left);
                const img = new Image();
                img.onload = () => {
                    this.log('[ChickenManager] Main image loaded for animation');
                ...
                img.onerror = (err) => {
                    this.error('[ChickenManager] Failed to load image for animation:', err);
                };
                img.src = leftImgPath;
                return;
            }
            ...
            this.error('[ChickenManager] Error in startSpriteAnimation:', error);
        }
    }
    ...
    initializeAnimation(element, canvas, ctx, isFlipped) {
        try {
            ...
            if (!element || !element.isConnected) {
                this.error('[ChickenManager] Element no longer connected to DOM');
                return;
            }
            
            const chickenData = this.animatedChickens.get(element);
            if (!chickenData) {
                this.error('[ChickenManager] Chicken data not found in initializeAnimation');
                return;
            }
            
            // check if sprite sheets are loaded
            if (!this.loadedAnimations || Object.keys(this.loadedAnimations).length === 0) {
                this.error('[ChickenManager] Sprite sheets not loaded');
                return;
            }
            
            // at least one default image is required
            if (!this.loadedAnimations.default || !this.loadedAnimations.default.complete) {
                this.error('[ChickenManager] Default sprite sheet not loaded or incomplete');
                return;
            }
            
            // Clear any existing animation
            if (chickenData.animationInterval) {
                clearInterval(chickenData.animationInterval);
                chickenData.animationInterval = null;
            }
            ...
            if (canvas.width !== elementWidth * 1.2 || canvas.height !== elementHeight * 1.2) {
                this.log(`[ChickenManager] Updating canvas size to match element: ${elementWidth}x${elementHeight}`);
            ...
            const isMoving = element.classList.contains('pet-moving') || element.classList.contains('chicken-moving');
            
            chickenData.currentFrame = 0;
            chickenData.animationState = isMoving ? 'walking' : 'idle';
            
            // immediately draw the first frame
            this.drawFrame(ctx, canvas, this.loadedAnimations, chickenData.currentFrame);
            
            // set animation interval
            const frameDuration = this.config.animationSettings.frameDuration;
            
            chickenData.animationInterval = setInterval(() => {
                // check if element is still connected to DOM
                if (!element.isConnected) {
                    if (chickenData.animationInterval) {
                        clearInterval(chickenData.animationInterval);
                ...
                const isCurrentlyMoving = element.classList.contains('pet-moving') || 
                                          element.classList.contains('chicken-moving');
                
                // update animation based on current state
                if (isCurrentlyMoving) {
                    // when in moving state, play the complete walking animation
                    if (chickenData.animationState !== 'walking') {
                        // state changed from idle to moving, reset to the 0th frame
                        chickenData.currentFrame = 0;
                        chickenData.animationState = 'walking';
                    } else {
                        // loop within the walking frame range
                        const {start, end} = this.config.animationSettings.walkingFrames;
                        chickenData.currentFrame++;
                        
                        // loop back to the start frame
                        if (chickenData.currentFrame > end) {
                            chickenData.currentFrame = start;
                        }
                    }
                } else {
                    // when not in moving state, only display idle frame
                    if (chickenData.animationState !== 'idle') {
                        chickenData.currentFrame = this.config.animationSettings.idleFrames.start;
                        chickenData.animationState = 'idle';
                    }
                ...
                if (Math.abs(currentWidth - elementWidth) > elementWidth * 0.1 ||
                    ...
                    this.log(`[ChickenManager] Element size changed, updated canvas: ${canvas.width}x${canvas.height}`);
                }
            ...
            this.error('[ChickenManager] Error in initializeAnimation:', error);
        }
    }
    ...
    getCurrentRotationTestConfig() {
        ...
        if (now - this.rotationTestConfig.lastChangeTime > this.rotationTestConfig.timePerTest) {
            ...
            console.log(`[ChickenManager] Test rotation configuration #${this.rotationTestConfig.currentTest}: ${currentConfig.description}`);
        }
    ...
    drawFrame(ctx, canvas, spriteSheets, frameIndex) {
        ...
        if (!frameData) {
            this.error(`[ChickenManager] Frame data not found for: ${frameName}`);
            return;
        }
        ...
        try {
            ...
            this.error('[ChickenManager] Error drawing frame:', error);
        }
    }
    
    pauseAllAnimations() {
        this.animatedChickens.forEach((chickenData, element) => {
            if (chickenData.animationInterval) {
                clearInterval(chickenData.animationInterval);
                chickenData.animationInterval = null;
            }
    ...
    resumeAllAnimations() {
        this.animatedChickens.forEach((chickenData, element) => {
            if (!chickenData.animationInterval && element.isConnected) {
                // Restart animation
                const animContainer = element.querySelector('.chicken-animation-container');
                if (animContainer) {
                    const canvas = animContainer.querySelector('.chicken-animation-canvas');
                    if (canvas) {
                        this.startChickenAnimation(element, chickenData.isFlipped);
                    }
                }
            }
        });
    ...
    cleanupNonExistentAnimations(currentNftIds) {
        ...
        elementsToRemove.forEach(element => {
            this.log(`[ChickenManager] Cleaning up animation for removed element: ${element.dataset.id || element.dataset.name}`);
    ...
    updateChickenDirection(element, isRight) {
        this.log(`[ChickenManager] Updating chicken direction: ${isRight ? 'right' : 'left'}`);
        ...
        const container = element.querySelector('.chicken-animation-container');
        if (container) {
            if (isRight) {
    ...
    cleanupAnimation(element) {
        ...
        if (chickenData.positionObserver) {
            clearInterval(chickenData.positionObserver);
        }
        ...
        this.animatedChickens.delete(element);
    }
    ...
    cleanupAllAnimations() {
        this.animatedChickens.forEach((_, element) => {
            this.cleanupAnimation(element);
        });
    }
     ...
     */
    resetProcessedState() {
        this.log('[ChickenManager] Resetting processed state, will recheck all NFTs');
        this.processedNftIds.clear();
    ...
    setupDirectionChangeListener() {
        ...
        document.addEventListener('pet-direction-changed', (event) => {
            ...
            if (this.isChickenNFTElement(element)) {
                this.updateChickenDirection(element, direction === 'right');
            }
        });
        
        // if PetMovementManager is available, listen to its direction change event
        if (window.PetMovementManager) {
            this.log('[ChickenManager] Adding PetMovementManager direction change hook');
            ...
            if (typeof originalUpdateDirectionState === 'function') {
                window.PetMovementManager.updateDirectionState = (pet) => {
                    ...
                    // then check if it is a chicken and update direction
                    if (this.isChickenNFTElement(pet.element)) {
                        // get direction information
                        const isFacingRight = pet.state.direction.x > 0;
                        
                        // delay update to ensure state has been updated by the original method
                        setTimeout(() => {
                            this.updateChickenDirection(pet.element, isFacingRight);
                        }, 10);
                    }
                };
            }
...
// Create and initialize the chicken manager
const chickenManager = new ChickenManager();
...
window.addEventListener('nftRefreshed', () => {
    if (chickenManager.initialized) {
        // When NFTs are refreshed, reset processed state and recheck
        chickenManager.resetProcessedState();
        // Delay slightly to ensure DOM has updated
        setTimeout(() => chickenManager.checkForChickens(), 500);
    }
});

// Export the chicken manager for use in other modules
window.ChickenManager = chickenManager;

// Add the backward compatibility method checkForChickens just like in egg.js
// First check if this is already using PetAnimationBase
if (ChickenManager.prototype instanceof PetAnimationBase) {
    /**
     * Backward compatibility method - calls checkForPets
     * @param {boolean} applyShadows - Whether to apply shadows
     */
    ChickenManager.prototype.checkForChickens = function(applyShadows = true) {
