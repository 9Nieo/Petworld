...
// CatManager class to handle all cat-related functionality
class CatManager {
    constructor() {
        ...
        this.config = {
            ...
            animations: {
                left: '../../resources/actions/Cat/left.png',
                leftAtlas: '../../resources/actions/Cat/left.atlas'
            },
            // Animation settings
            animationSettings: {
            ...
            shadow: {
                ...
                applyToExisting: true    // Whether to apply to existing cats
            }
        };
    ...
    /**
     * Initialize the Cat Manager
     */
    async init() {
        if (this.initialized) return;
        
        this.log('[CatManager] Initializing cat animation system...');
        
        // Load and parse the atlas file
        try {
            await this.loadAtlasData();
        } catch (error) {
            this.error('[CatManager] Failed to load atlas data:', error);
        }
        
        // Listen for NFT loaded events to detect cats
        ...
        // Check for cats immediately if we're on the main page
        if (window.location.href.includes('home.html')) {
            // Short delay to ensure DOM is fully loaded
            setTimeout(() => this.checkForCats(), 1000);
        }
        
        // Setup periodic cat checking on main page
        this.setupPeriodicCatCheck();
    ...
    initShadowSystem() {
        ...
        this.log('[CatManager] Initializing shadow system...');
        
        // Check if PetShadowManager is available
        if (window.PetShadowManager) {
            this.log('[CatManager] PetShadowManager found, will use for cat shadows');
            
            if (this.config.shadow.applyToExisting) {
                setTimeout(() => {
                    this.applyAllCatShadows();
                }, 1500);
            }
            
            document.addEventListener('cat-moving-state-changed', (event) => {
                if (event.detail && event.detail.element) {
                    const catElement = event.detail.element;
                    
                    // Update shadow state
                    if (window.PetShadowManager && window.PetShadowManager.updateShadow) {
                        window.PetShadowManager.updateShadow(catElement);
                    }
                }
            });
        } else {
            this.log('[CatManager] PetShadowManager not available, shadows will not be applied');
        }
    }
    ...
    applyAllCatShadows() {
        ...
        this.log('[CatManager] Applying shadows to all cats...');
        
        const catElements = document.querySelectorAll('.cat-nft');
        let count = 0;
        
        catElements.forEach(catElement => {
            // Add shadow
            window.PetShadowManager.addShadow(catElement, {
                ratio: 0.8,  // Shadow width relative to cat
                jumpEffect: true  // Enable jumping effect
            });
            count++;
        });
        
        this.log(`[CatManager] Applied shadows to ${count} cats`);
    }
    ...
    async loadAtlasData() {
        ...
        try {
            ...
            this.log(`[CatManager] Atlas path: ${atlasPath}`);
            ...
            this.log('[CatManager] Atlas data parsed using SpriteAtlasHelper');
            
            // get all image files to load
            const imageFiles = this.atlasData.meta.images.map(img => img.file);
            this.log(`[CatManager] Atlas references ${imageFiles.length} image files:`, imageFiles);
            ...
            this.log('[CatManager] Atlas data and all images loaded successfully');
        } catch (error) {
            this.error('[CatManager] Error loading atlas file:', error);
    ...
    async preloadAllAnimations(basePath, imageFiles) {
        try {
            ...
            this.log(`[CatManager] Successfully loaded ${Object.keys(this.loadedAnimations).length - 1} sprite sheets`);
        } catch (error) {
            this.error('[CatManager] Error preloading animations:', error);
    ...
    preloadAnimations() {
        ...
        if (!this.loadedAnimations || Object.keys(this.loadedAnimations).length === 0) {
            ...
            leftImg.onload = () => {
                this.log(`[CatManager] Main image preloaded successfully`);
                this.loadedAnimations = { default: leftImg, 0: leftImg };
            };
            leftImg.onerror = (err) => {
                this.error(`[CatManager] Error preloading main image:`, err);
            };
            
            // Get full path
            const imagePath = this.getAbsoluteResourcePath(this.config.animations.left);
            this.log(`[CatManager] Main image path: ${imagePath}`);
    ...
    setupPeriodicCatCheck() {
        if (window.location.href.includes('home.html')) {
            setInterval(() => {
                this.checkForCats(false); // Pass false to indicate not to auto-apply shadows
            }, 10000);
        }
    }
    ...
    /**
     * Observe the NFT container for new cats
     */
    observeNFTContainer() {
        ...
        const observer = new MutationObserver((mutations) => {
            let shouldCheckCats = false;
            
            mutations.forEach(mutation => {
                if (mutation.type === 'childList' && mutation.addedNodes.length > 0) {
                    shouldCheckCats = true;
                }
            });
        ...
        this.log('[CatManager] Now monitoring farm container for new NFTs');
    }
    ...
    createFallbackAtlasData() {
        this.log('[CatManager] Creating fallback atlas data');
    ...
    handleNFTLoadedEvent(event) {
        ...
        if (!nftElement.isConnected) {
            this.log(`[CatManager] NFT element not connected to DOM, skipping processing`);
            return;
        }
        ...
        if (this.processedNftIds.has(nftId)) {
            this.log(`[CatManager] NFT already processed, skipping: ${nftId}`);
            return;
        }
        
        // If element already has animation, skip
        if (this.animatedCats.has(nftElement)) {
            this.log(`[CatManager] Element already animated, skipping: ${nftId}`);
            return;
        }
        
        // Check if this is a cat
        if (this.isCatNFT(nftData)) {
            this.log('[CatManager] Cat NFT detected from event:', nftData.metadata?.name || nftData.name || nftId);
            
            // Mark this NFT as processed
            this.processedNftIds.add(nftId);
            
            // Apply animation
            this.applyCatAnimation(nftElement);
        }
    }
    
    /**
     * Check the page for cat NFTs and apply animations
     * @param {boolean} applyShadows - Whether to apply shadows, default is true
     */
    checkForCats(applyShadows = true) {
        this.log('[CatManager] Checking for cats on the page...');
        
        // Find all NFT elements in the farm
        const farmContainer = document.getElementById('farm-animals-container');
        if (!farmContainer) {
            this.error('[CatManager] Farm container not found');
            return;
        }
        
        // Find all animal elements
        const animalElements = farmContainer.querySelectorAll('.animal');
        this.log(`[CatManager] Found ${animalElements.length} animal elements`);
        ...
        animalElements.forEach((animalElement, index) => {
            ...
            if (this.animatedCats.has(animalElement)) {
                this.log(`[CatManager] Element already animated, skipping: ${nftId}`);
                return;
            }
            
            // Check if this is a cat
            if (this.isCatNFTElement(animalElement)) {
                this.log(`[CatManager] Found cat NFT #${index} (ID: ${nftId}):`, animalElement.dataset.name);
                
                // Mark this NFT as processed
                this.processedNftIds.add(nftId);
                
                // Apply animation
                this.applyCatAnimation(animalElement);
            }
        });
    ...
    isCatNFTElement(element) {
        // Check if the element has dataset
        if (!element.dataset) return false;
        
        // Check for cat name, type, or class
        const name = element.dataset.name || '';
        const type = element.dataset.type || '';
        
        const hasCatName = name.toLowerCase().includes('cat');
        const hasCatType = type.toLowerCase() === 'cat';
        const hasCatClass = element.classList.contains('cat-nft') || element.classList.contains('cat');
        
        return hasCatName || hasCatType || hasCatClass;
    }
    ...
    isCatNFT(nftData) {
        ...
        if (nftData.metadata && nftData.metadata.name) {
            return nftData.metadata.name.toLowerCase().includes('cat');
        }
        
        // Check direct name property
        if (nftData.name) {
            return nftData.name.toLowerCase().includes('cat');
        }
        
        // Check any other properties that could identify a cat
        if (nftData.type) {
            return nftData.type.toLowerCase() === 'cat';
        }
        
        return false;
    }

    applyCatAnimation(element) {
        if (this.animatedCats.has(element)) return;
        
        this.log('[CatManager] Applying cat animation to element:', element);
    ...
    /**
     * Actual method to apply cat animation (separated to support delayed processing)
     * @param {HTMLElement} element - The NFT element to animate
     */
    doApplyCatAnimation(element) {
        ...
        element.classList.add('cat-nft');
        
        this.animatedCats.set(element, {
            element,
            isFlipped,
        ...
        if (!success) {
            this.error('[CatManager] Failed to prepare element for animation');
            return;
        }
        
        // start animation
        this.startCatAnimation(element, isFlipped);
        
        // apply shadow - ensure shadow is applied before setting movement
        if (this.config.shadow.enabled) {
            this.applyCatShadow(element);
        }
        ...
        this.updateCatDirection(element, isFlipped);
        
        this.log(`[CatManager] Cat animation applied for: ${element.dataset.name || 'unnamed cat'}`);
    }
    
    applyCatShadow(element) {
        if (!this.config.shadow.enabled || !window.PetShadowManager) return;
        
        // If already in tracking map, check if already has shadow
        const catData = this.animatedCats.get(element);
        if (catData && catData.hasShadow) {
            this.log('[CatManager] Cat already has shadow, skipping:', element.dataset.name || element.dataset.id || 'unnamed');
            return;
        }
        
        // Add cat-nft class to support shadow selector
        element.classList.add('cat-nft');
        ...
        if (catData) {
            catData.hasShadow = true;
            this.animatedCats.set(element, catData);
        }
        
        this.log('[CatManager] Applied shadow to cat:', element.dataset.name || element.dataset.id || 'unnamed');
    }
    
    /**
     * Enable movement for cats
     * @param {HTMLElement} element - Cat element
     */
    enableMovement(element) {
        if (!window.PetMovementManager) {
            this.log('[CatManager] PetMovementManager not available, cannot enable movement');
            return;
        }
        ...
        const positionObserver = setInterval(() => {
            ...
            if (Math.abs(deltaX) > 0.5) {
                ...
                this.updateCatDirection(element, isMovingRight);
                lastX = currentX;
            }
        }, 100); 
        
        const catData = this.animatedCats.get(element) || {};
        catData.positionObserver = positionObserver;
        this.animatedCats.set(element, catData);
        ...
        window.PetMovementManager.enableMovement(element, {
            ...
            onStateChange: (isMoving) => {
                ...
                const catEvent = new CustomEvent('cat-moving-state-changed', {
                    detail: {
                        element: element,
                        isMoving: isMoving
                    },
                ...
                if (isMoving) {
                    element.classList.add('cat-moving');
                    element.classList.add('pet-moving');  // Also add generic moving class
                } else {
                    element.classList.remove('cat-moving');
        ...
        const directionHandler = (event) => {
            if (event.detail && event.detail.element === element) {
                ...
                this.updateCatDirection(element, isRight);
            }
        };
        ...
        this.log(`[CatManager] Enabled movement for cat: ${element.dataset.name || 'unnamed'} with speed ${speed.toFixed(2)}`);
    }
    ...
    prepareElementForAnimation(element, isFlipped) {
        ...
        try {
            ...
            const oldContainer = element.querySelector('.cat-animation-container');
            if (oldContainer) {
            ...
            animContainer.className = 'cat-animation-container';
            ...
            animContainer.style.zIndex = '10'; // Show above the cat image
            ...
            if (shouldFlip) {
                this.log('[CatManager] Setting initial animation direction: right (flipped)');
                ...
                this.log('[CatManager] Setting initial animation direction: left (normal)');
            ...
            this.log(`[CatManager] Element dimensions: ${elementWidth}x${elementHeight}`); 
            
            // Create canvas for sprite animation
            const canvas = document.createElement('canvas');
            canvas.className = 'cat-animation-canvas';
            ...
            this.error('[CatManager] Error preparing element for animation:', error);
            return false;
        }
    }
     ...
     */
    startCatAnimation(element, isFlipped) {
        // Check element
        if (!element || !element.isConnected) {
            this.error('[CatManager] Cannot start animation - element is not connected to DOM');
            return;
        }
        
        try {
            // Get animation container
            const animContainer = element.querySelector('.cat-animation-container');
            if (!animContainer) {
                this.error('[CatManager] Animation container not found');
                return;
            }
            
            // Get the canvas
            const canvas = animContainer.querySelector('.cat-animation-canvas');
            if (!canvas) {
                this.error('[CatManager] Canvas not found');
                return;
            }
            
            const catData = this.animatedCats.get(element);
            if (!catData) {
                this.error('[CatManager] Cat data not found');
                return;
            }
            
            // Wait until the atlas data is loaded
            if (!this.atlasData) {
                this.log('[CatManager] Atlas data not loaded yet, creating fallback');
                this.createFallbackAtlasData();
            }
            ...
            this.error('[CatManager] Error starting cat animation:', error);
        }
    }
    ...
    startSpriteAnimation(element, canvas, isFlipped) {
        try {
            // Get cat data from tracking map
            const catData = this.animatedCats.get(element);
            if (!catData) return;
            ...
            if (!ctx) {
                this.error('[CatManager] Failed to get canvas context');
                return;
            }
            
            // check if sprite sheets are loaded
            if (!this.loadedAnimations || Object.keys(this.loadedAnimations).length === 0) {
                this.log('[CatManager] Sprite sheets not yet loaded, trying to load now...');
                
                // try to load main image from the path in config
                const leftImgPath = this.getAbsoluteResourcePath(this.config.animations.left);
                const img = new Image();
                img.onload = () => {
                    this.log('[CatManager] Main image loaded for animation');
                ...
                img.onerror = (err) => {
                    this.error('[CatManager] Failed to load image for animation:', err);
                };
                img.src = leftImgPath;
                return;
            }
            ...
            this.error('[CatManager] Error in startSpriteAnimation:', error);
        }
    }
    ...
    initializeAnimation(element, canvas, ctx, isFlipped) {
        try {
            ...
            if (!element || !element.isConnected) {
                this.error('[CatManager] Element no longer connected to DOM');
                return;
            }
            
            const catData = this.animatedCats.get(element);
            if (!catData) {
                this.error('[CatManager] Cat data not found in initializeAnimation');
                return;
            }
            
            // check if sprite sheets are loaded
            if (!this.loadedAnimations || Object.keys(this.loadedAnimations).length === 0) {
                this.error('[CatManager] Sprite sheets not loaded');
                return;
            }
            
            // at least one default image is required
            if (!this.loadedAnimations.default || !this.loadedAnimations.default.complete) {
                this.error('[CatManager] Default sprite sheet not loaded or incomplete');
                return;
            }
            
            // Clear any existing animation
            if (catData.animationInterval) {
                clearInterval(catData.animationInterval);
                catData.animationInterval = null;
            }
            ...
            if (canvas.width !== elementWidth * 1.2 || canvas.height !== elementHeight * 1.2) {
                this.log(`[CatManager] Updating canvas size to match element: ${elementWidth}x${elementHeight}`);
            ...
            const isMoving = element.classList.contains('pet-moving') || element.classList.contains('cat-moving');
            
            catData.currentFrame = 0;
            catData.animationState = isMoving ? 'walking' : 'idle';
            
            // immediately draw the first frame
            this.drawFrame(ctx, canvas, this.loadedAnimations, catData.currentFrame);
            
            // set animation interval
            const frameDuration = this.config.animationSettings.frameDuration;
            
            catData.animationInterval = setInterval(() => {
                // check if element is still connected to DOM
                if (!element.isConnected) {
                    if (catData.animationInterval) {
                        clearInterval(catData.animationInterval);
                ...
                const isCurrentlyMoving = element.classList.contains('pet-moving') || 
                                          element.classList.contains('cat-moving');
                
                // update animation based on current state
                if (isCurrentlyMoving) {
                    // when in moving state, play the complete walking animation
                    if (catData.animationState !== 'walking') {
                        // state changed from idle to moving, reset to the 0th frame
                        catData.currentFrame = 0;
                        catData.animationState = 'walking';
                    } else {
                        // loop within the walking frame range
                        const {start, end} = this.config.animationSettings.walkingFrames;
                        catData.currentFrame++;
                        
                        // loop back to the start frame
                        if (catData.currentFrame > end) {
                            catData.currentFrame = start;
                        }
                    }
                } else {
                    // when not in moving state, only display idle frame
                    if (catData.animationState !== 'idle') {
                        catData.currentFrame = this.config.animationSettings.idleFrames.start;
                        catData.animationState = 'idle';
                    }
                ...
                if (Math.abs(currentWidth - elementWidth) > elementWidth * 0.1 ||
                    ...
                    this.log(`[CatManager] Element size changed, updated canvas: ${canvas.width}x${canvas.height}`);
                }
            ...
            this.error('[CatManager] Error in initializeAnimation:', error);
        }
    }
    ...
    getCurrentRotationTestConfig() {
        ...
        if (now - this.rotationTestConfig.lastChangeTime > this.rotationTestConfig.timePerTest) {
            ...
            console.log(`[CatManager] Test rotation configuration #${this.rotationTestConfig.currentTest}: ${currentConfig.description}`);
        }
    ...
    drawFrame(ctx, canvas, spriteSheets, frameIndex) {
        ...
        if (!frameData) {
            this.error(`[CatManager] Frame data not found for: ${frameName}`);
            return;
        }
        ...
        try {
            ...
            this.error('[CatManager] Error drawing frame:', error);
        }
    }
    
    pauseAllAnimations() {
        this.animatedCats.forEach((catData, element) => {
            if (catData.animationInterval) {
                clearInterval(catData.animationInterval);
                catData.animationInterval = null;
            }
    ...
    resumeAllAnimations() {
        this.animatedCats.forEach((catData, element) => {
            if (!catData.animationInterval && element.isConnected) {
                // Restart animation
                const animContainer = element.querySelector('.cat-animation-container');
                if (animContainer) {
                    const canvas = animContainer.querySelector('.cat-animation-canvas');
                    if (canvas) {
                        this.startCatAnimation(element, catData.isFlipped);
                    }
                }
            }
        });
    ...
    cleanupNonExistentAnimations(currentNftIds) {
        ...
        elementsToRemove.forEach(element => {
            this.log(`[CatManager] Cleaning up animation for removed element: ${element.dataset.id || element.dataset.name}`);
    ...
    updateCatDirection(element, isRight) {
        this.log(`[CatManager] Updating cat direction: ${isRight ? 'right' : 'left'}`);
        ...
        const container = element.querySelector('.cat-animation-container');
        if (container) {
            if (isRight) {
    ...
    cleanupAnimation(element) {
        ...
        if (catData.positionObserver) {
            clearInterval(catData.positionObserver);
        }
        ...
        this.animatedCats.delete(element);
    }
    ...
    cleanupAllAnimations() {
        this.animatedCats.forEach((_, element) => {
            this.cleanupAnimation(element);
        });
    }
     ...
     */
    resetProcessedState() {
        this.log('[CatManager] Resetting processed state, will recheck all NFTs');
        this.processedNftIds.clear();
    ...
    setupDirectionChangeListener() {
        ...
        document.addEventListener('pet-direction-changed', (event) => {
            ...
            if (this.isCatNFTElement(element)) {
                this.updateCatDirection(element, direction === 'right');
            }
        });
        
        // if PetMovementManager is available, listen to its direction change event
        if (window.PetMovementManager) {
            this.log('[CatManager] Adding PetMovementManager direction change hook');
            ...
            if (typeof originalUpdateDirectionState === 'function') {
                window.PetMovementManager.updateDirectionState = (pet) => {
                    ...
                    // then check if it is a cat and update direction
                    if (this.isCatNFTElement(pet.element)) {
                        // get direction information
                        const isFacingRight = pet.state.direction.x > 0;
                        
                        // delay update to ensure state has been updated by the original method
                        setTimeout(() => {
                            this.updateCatDirection(pet.element, isFacingRight);
                        }, 10);
                    }
                };
            }
...
// Create and initialize the cat manager
...
window.addEventListener('nftRefreshed', () => {
    if (catManager.initialized) {
        // When NFTs are refreshed, reset processed state and recheck
        catManager.resetProcessedState();
        // Delay slightly to ensure DOM has updated
        setTimeout(() => catManager.checkForCats(), 500);
    }
});

// Export the cat manager for use in other modules
window.CatManager = catManager;

// Add the backward compatibility method checkForCats just like in egg.js
// First check if this is already using PetAnimationBase
if (CatManager.prototype instanceof PetAnimationBase) {
    /**
     * Backward compatibility method - calls checkForPets
     * @param {boolean} applyShadows - Whether to apply shadows
     */
    CatManager.prototype.checkForCats = function(applyShadows = true) {
