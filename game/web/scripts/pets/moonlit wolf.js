// MoonlitWolfManager class to handle all moonlit wolf-related functionality
class MoonlitWolfManager {
    constructor() {
        // Configuration settings
        this.config = {
            // Animation paths - using relative paths
            animations: {
                left: '../../resources/actions/Moonlit wolf/left.png',
                leftAtlas: '../../resources/actions/Moonlit wolf/left.atlas'
            },
            // Animation settings
            animationSettings: {
                frameDuration: 15,      // Duration of each frame in milliseconds
                loopDelay: 0,           // Delay between animation loops in milliseconds
                totalFrames: 53,        // total frames (0-28)
                walkingFrames: {         // walking animation frame range
                    start: 0,            // walking animation start frame
                    end: 52             // walking animation end frame
                },
            ...
            shadow: {
                ...
                applyToExisting: true    // Whether to apply to existing moonlit wolves
            }
        };
        ...
        // Track animated moonlit wolves
        this.animatedMoonlitWolves = new Map();
    ...
    /**
     * Initialize the Moonlit Wolf Manager
     */
    async init() {
        if (this.initialized) return;
        
        this.log('[MoonlitWolfManager] Initializing moonlit wolf animation system...');
        
        // Load and parse the atlas file
        try {
            await this.loadAtlasData();
        } catch (error) {
            this.error('[MoonlitWolfManager] Failed to load atlas data:', error);
        }
        
        // Listen for NFT loaded events to detect moonlit wolves
        ...
        // Check for moonlit wolves immediately if we're on the main page
        if (window.location.href.includes('home.html')) {
            // Short delay to ensure DOM is fully loaded
            setTimeout(() => this.checkForMoonlitWolves(), 1000);
        }
        
        // Setup periodic moonlit wolf checking on main page
        this.setupPeriodicMoonlitWolfCheck();
    ...
    initShadowSystem() {
        ...
        this.log('[MoonlitWolfManager] Initializing shadow system...');
        
        // Check if PetShadowManager is available
        if (window.PetShadowManager) {
            this.log('[MoonlitWolfManager] PetShadowManager found, will use for moonlit wolf shadows');
            
            if (this.config.shadow.applyToExisting) {
                setTimeout(() => {
                    this.applyAllMoonlitWolfShadows();
                }, 1500);
            }
            
            document.addEventListener('moonlit-wolf-moving-state-changed', (event) => {
                if (event.detail && event.detail.element) {
                    const moonlitWolfElement = event.detail.element;
                    
                    // Update shadow state
                    if (window.PetShadowManager && window.PetShadowManager.updateShadow) {
                        window.PetShadowManager.updateShadow(moonlitWolfElement);
                    }
                }
            });
        } else {
            this.log('[MoonlitWolfManager] PetShadowManager not available, shadows will not be applied');
        }
    }
    
    applyAllMoonlitWolfShadows() {
        if (!window.PetShadowManager) return;
        
        this.log('[MoonlitWolfManager] Applying shadows to all moonlit wolves...');
        
        const moonlitWolfElements = document.querySelectorAll('.moonlit-wolf-nft');
        let count = 0;
        
        moonlitWolfElements.forEach(moonlitWolfElement => {
            // Add shadow
            window.PetShadowManager.addShadow(moonlitWolfElement, {
                ratio: 0.8,  // Shadow width relative to moonlit wolf
                jumpEffect: true  // Enable jumping effect
            });
            count++;
        });
        
        this.log(`[MoonlitWolfManager] Applied shadows to ${count} moonlit wolves`);
    }
    ...
    async loadAtlasData() {
        ...
        try {
            ...
            this.log(`[MoonlitWolfManager] Atlas path: ${atlasPath}`);
            ...
            this.log('[MoonlitWolfManager] Atlas data parsed using SpriteAtlasHelper');
            
            // get all image files to load
            const imageFiles = this.atlasData.meta.images.map(img => img.file);
            this.log(`[MoonlitWolfManager] Atlas references ${imageFiles.length} image files:`, imageFiles);
            ...
            this.log('[MoonlitWolfManager] Atlas data and all images loaded successfully');
        } catch (error) {
            this.error('[MoonlitWolfManager] Error loading atlas file:', error);
    ...
    async preloadAllAnimations(basePath, imageFiles) {
        try {
            ...
            this.log(`[MoonlitWolfManager] Successfully loaded ${Object.keys(this.loadedAnimations).length - 1} sprite sheets`);
        } catch (error) {
            this.error('[MoonlitWolfManager] Error preloading animations:', error);
    ...
    preloadAnimations() {
        ...
        if (!this.loadedAnimations || Object.keys(this.loadedAnimations).length === 0) {
            ...
            leftImg.onload = () => {
                this.log(`[MoonlitWolfManager] Main image preloaded successfully`);
                this.loadedAnimations = { default: leftImg, 0: leftImg };
            };
            leftImg.onerror = (err) => {
                this.error(`[MoonlitWolfManager] Error preloading main image:`, err);
            };
            
            // Get full path
            const imagePath = this.getAbsoluteResourcePath(this.config.animations.left);
            this.log(`[MoonlitWolfManager] Main image path: ${imagePath}`);
    ...
    setupPeriodicMoonlitWolfCheck() {
        if (window.location.href.includes('home.html')) {
            setInterval(() => {
                this.checkForMoonlitWolves(false); // Pass false to indicate not to auto-apply shadows
            }, 10000);
        }
    }
    ...
    /**
     * Observe the NFT container for new moonlit wolves
     */
    observeNFTContainer() {
        ...
        const observer = new MutationObserver((mutations) => {
            let shouldCheckMoonlitWolves = false;
            
            mutations.forEach(mutation => {
                if (mutation.type === 'childList' && mutation.addedNodes.length > 0) {
                    shouldCheckMoonlitWolves = true;
                }
            });
            
            if (shouldCheckMoonlitWolves) {
                this.checkForMoonlitWolves();
            }
        });
        ...
        this.log('[MoonlitWolfManager] Now monitoring farm container for new NFTs');
    }
    ...
    createFallbackAtlasData() {
        this.log('[MoonlitWolfManager] Creating fallback atlas data');
    ...
    handleNFTLoadedEvent(event) {
        ...
        if (!nftElement.isConnected) {
            this.log(`[MoonlitWolfManager] NFT element not connected to DOM, skipping processing`);
            return;
        }
        ...
        if (this.processedNftIds.has(nftId)) {
            this.log(`[MoonlitWolfManager] NFT already processed, skipping: ${nftId}`);
            return;
        }
        
        // If element already has animation, skip
        if (this.animatedMoonlitWolves.has(nftElement)) {
            this.log(`[MoonlitWolfManager] Element already animated, skipping: ${nftId}`);
            return;
        }
        
        // Check if this is a moonlit wolf
        if (this.isMoonlitWolfNFT(nftData)) {
            this.log('[MoonlitWolfManager] Moonlit wolf NFT detected from event:', nftData.metadata?.name || nftData.name || nftId);
            
            // Mark this NFT as processed
            this.processedNftIds.add(nftId);
            
            // Apply animation
            this.applyMoonlitWolfAnimation(nftElement);
        }
    }
    
    /**
     * Check the page for moonlit wolf NFTs and apply animations
     * @param {boolean} applyShadows - Whether to apply shadows, default is true
     */
    checkForMoonlitWolves(applyShadows = true) {
        this.log('[MoonlitWolfManager] Checking for moonlit wolves on the page...');
        
        // Find all NFT elements in the farm
        const farmContainer = document.getElementById('farm-animals-container');
        if (!farmContainer) {
            this.error('[MoonlitWolfManager] Farm container not found');
            return;
        }
        
        // Find all animal elements
        const animalElements = farmContainer.querySelectorAll('.animal');
        this.log(`[MoonlitWolfManager] Found ${animalElements.length} animal elements`);
        ...
        animalElements.forEach((animalElement, index) => {
            ...
            if (this.animatedMoonlitWolves.has(animalElement)) {
                this.log(`[MoonlitWolfManager] Element already animated, skipping: ${nftId}`);
                return;
            }
            
            // Check if this is a moonlit wolf
            if (this.isMoonlitWolfNFTElement(animalElement)) {
                this.log(`[MoonlitWolfManager] Found moonlit wolf NFT #${index} (ID: ${nftId}):`, animalElement.dataset.name);
                
                // Mark this NFT as processed
                this.processedNftIds.add(nftId);
                
                // Apply animation
                this.applyMoonlitWolfAnimation(animalElement);
            }
        });
    ...
    isMoonlitWolfNFTElement(element) {
        // Check if the element has dataset
        if (!element.dataset) return false;
        
        // Check for moonlit wolf name, type, or class
        const name = element.dataset.name || '';
        const type = element.dataset.type || '';
        
        const hasMoonlitWolfName = name.toLowerCase().includes('moonlit wolf');
        const hasMoonlitWolfType = type.toLowerCase() === 'moonlit wolf';
        const hasMoonlitWolfClass = element.classList.contains('moonlit-wolf-nft') || element.classList.contains('moonlit wolf');
        
        return hasMoonlitWolfName || hasMoonlitWolfType || hasMoonlitWolfClass;
    }
    
    isMoonlitWolfNFT(nftData) {
        if (!nftData) return false;
        
        // Check metadata name
        if (nftData.metadata && nftData.metadata.name) {
            return nftData.metadata.name.toLowerCase().includes('moonlit wolf');
        }
        
        // Check direct name property
        if (nftData.name) {
            return nftData.name.toLowerCase().includes('moonlit wolf');
        }
        
        // Check any other properties that could identify a moonlit wolf
        if (nftData.type) { 
            return nftData.type.toLowerCase() === 'moonlit wolf';
        }
        
        return false;
    }

    applyMoonlitWolfAnimation(element) {
        if (this.animatedMoonlitWolves.has(element)) return;
        
        this.log('[MoonlitWolfManager] Applying moonlit wolf animation to element:', element);
        
        // Check if element is visible and has dimensions
        if (!element.isConnected) {
            this.error('[MoonlitWolfManager] Element is not connected to DOM, cannot animate:', element);
            return;
        }
        ...
        if (window.PetSizeHelper) {
            window.PetSizeHelper.applyAnimationWithSizeCheck(
                element,
                'moonlit wolf',
                (el) => this.doApplyMoonlitWolfAnimation(el),
                {
                    maxWaitAttempts: 8,
            ...
            this.log('[MoonlitWolfManager] PetSizeHelper not available, using fallback logic');
            
            // Check element dimensions, delay processing if zero
            if (element.offsetWidth === 0 || element.offsetHeight === 0) {
                this.log('[MoonlitWolfManager] Element has zero dimensions, delaying animation:', element);
                ...
                setTimeout(() => {
                    ...
                    if (element.isConnected && element.offsetWidth > 0 && element.offsetHeight > 0) {
                        this.log('[MoonlitWolfManager] Element now has dimensions, applying animation');
                        this.doApplyMoonlitWolfAnimation(element);
                    } else {
                        this.error('[MoonlitWolfManager] Element still has zero dimensions after delay, applying PetSizeManager or default size');
                        ...
                        if (window.PetSizeManager) {
                            ...
                            this.log('[MoonlitWolfManager] Applied PetSizeManager sizing');
                        }
                        ...
                        if (element.offsetWidth === 0 || element.offsetHeight === 0) {
                            ...
                            this.log('[MoonlitWolfManager] Applied fallback size as last resort');
                        }
    ...
    /**
     * Actual method to apply moonlit wolf animation (separated to support delayed processing)
     * @param {HTMLElement} element - The NFT element to animate
     */
    doApplyMoonlitWolfAnimation(element) {
        ...
        element.classList.add('moonlit-wolf-nft');
        
        this.animatedMoonlitWolves.set(element, {
            element,
            isFlipped,
        ...
        if (!success) {
            this.error('[MoonlitWolfManager] Failed to prepare element for animation');
            return;
        }
        
        // start animation
        this.startMoonlitWolfAnimation(element, isFlipped);
        
        // apply shadow - ensure shadow is applied before setting movement
        if (this.config.shadow.enabled) {
            this.applyMoonlitWolfShadow(element);
        }
        ...
        this.updateMoonlitWolfDirection(element, isFlipped);
        
        this.log(`[MoonlitWolfManager] Moonlit wolf animation applied for: ${element.dataset.name || 'unnamed moonlit wolf'}`);
    }
    
    applyMoonlitWolfShadow(element) {
        if (!this.config.shadow.enabled || !window.PetShadowManager) return;
        
        // If already in tracking map, check if already has shadow
        const moonlitWolfData = this.animatedMoonlitWolves.get(element);
        if (moonlitWolfData && moonlitWolfData.hasShadow) {
            this.log('[MoonlitWolfManager] Moonlit wolf already has shadow, skipping:', element.dataset.name || element.dataset.id || 'unnamed');
            return;
        }
        
        // Add moonlit wolf-nft class to support shadow selector
        element.classList.add('moonlit-wolf-nft');
        ...
        if (moonlitWolfData) {
            moonlitWolfData.hasShadow = true;
            this.animatedMoonlitWolves.set(element, moonlitWolfData);
        }
        
        this.log('[MoonlitWolfManager] Applied shadow to moonlit wolf:', element.dataset.name || element.dataset.id || 'unnamed');
    }
    
    /**
     * Enable movement for moonlit wolves
     * @param {HTMLElement} element - Moonlit wolf element
     */
    enableMovement(element) {
        if (!window.PetMovementManager) {
            this.log('[MoonlitWolfManager] PetMovementManager not available, cannot enable movement');
            return;
        }
        ...
        const positionObserver = setInterval(() => {
            ...
            if (Math.abs(deltaX) > 0.5) {
                ...
                this.updateMoonlitWolfDirection(element, isMovingRight);
                lastX = currentX;
            }
        }, 100); 
        
        const moonlitWolfData = this.animatedMoonlitWolves.get(element) || {};
        moonlitWolfData.positionObserver = positionObserver;
        this.animatedMoonlitWolves.set(element, moonlitWolfData);
        ...
        window.PetMovementManager.enableMovement(element, {
            ...
            onStateChange: (isMoving) => {
                ...
                const moonlitWolfEvent = new CustomEvent('moonlit-wolf-moving-state-changed', {
                    detail: {
                        element: element,
                        isMoving: isMoving
                    },
        ...
        const directionHandler = (event) => {
            if (event.detail && event.detail.element === element) {
                ...
                this.updateMoonlitWolfDirection(element, isRight);
            }
        };
        ...
        this.log(`[MoonlitWolfManager] Enabled movement for moonlit wolf: ${element.dataset.name || 'unnamed'} with speed ${speed.toFixed(2)}`);
    }
    ...
    prepareElementForAnimation(element, isFlipped) {
        ...
        if (!element || !element.isConnected) {
            this.error('[MoonlitWolfManager] Cannot prepare element - element is not connected to DOM');
            return false;
        }
        ...
        this.log(`[Moonlit WolfManager] Element dimension check: offset(${offsetWidth}x${offsetHeight}), css(${cssWidth}x${cssHeight})`);
        
        // Apply size using PetSizeManager first if available
        if (window.PetSizeManager) {
            window.PetSizeManager.setSize(element, 'moonlit wolf');
            this.log('[Moonlit WolfManager] Applied size using PetSizeManager');
        }
        ...
        if (effectiveWidth === 0 || effectiveHeight === 0) {
            this.error('[Moonlit WolfManager] Element still has zero effective dimensions after PetSizeManager, applying fallback size');
        ...
        try {
            ...
            const oldContainer = element.querySelector('.moonlit-wolf-animation-container');
            if (oldContainer) {
            ...
            animContainer.className = 'moonlit-wolf-animation-container';
            ...
            animContainer.style.zIndex = '10'; // Show above the moonlit wolf image
            ...
            if (shouldFlip) {
                this.log('[MoonlitWolfManager] Setting initial animation direction: right (flipped)');
                ...
                this.log('[MoonlitWolfManager] Setting initial animation direction: left (normal)');
            ...
            this.log(`[MoonlitWolfManager] Element dimensions: ${elementWidth}x${elementHeight}`); 
            
            // Create canvas for sprite animation
            const canvas = document.createElement('canvas');
            canvas.className = 'moonlit-wolf-animation-canvas';
            ...
            this.error('[MoonlitWolfManager] Error preparing element for animation:', error);
            return false;
        }
    }
    
    /**
     * Start the moonlit wolf animation
     * @param {HTMLElement} element - The element to animate
     * @param {boolean} isFlipped - Whether the animation is flipped
     */
    startMoonlitWolfAnimation(element, isFlipped) {
        // Check element
        if (!element || !element.isConnected) {
            this.error('[MoonlitWolfManager] Cannot start animation - element is not connected to DOM');
            return;
        }
        
        try {
            // Get animation container
            const animContainer = element.querySelector('.moonlit-wolf-animation-container');
            if (!animContainer) {
                this.error('[MoonlitWolfManager] Animation container not found');
                return;
            }
            
            // Get the canvas
            const canvas = animContainer.querySelector('.moonlit-wolf-animation-canvas');
            if (!canvas) {
                this.error('[MoonlitWolfManager] Canvas not found');
                return;
            }
            
            const moonlitWolfData = this.animatedMoonlitWolves.get(element);
            if (!moonlitWolfData) {
                this.error('[MoonlitWolfManager] Moonlit wolf data not found');
                return;
            }
            
            // Wait until the atlas data is loaded
            if (!this.atlasData) {
                this.log('[MoonlitWolfManager] Atlas data not loaded yet, creating fallback');
                this.createFallbackAtlasData();
            }
            ...
            this.error('[MoonlitWolfManager] Error starting moonlit wolf animation:', error);
        }
    }
    ...
    startSpriteAnimation(element, canvas, isFlipped) {
        try {
            // Get moonlit wolf data from tracking map
            const moonlitWolfData = this.animatedMoonlitWolves.get(element);
            if (!moonlitWolfData) return;
            
            // Get the canvas context
            const ctx = canvas.getContext('2d');
            if (!ctx) {
                this.error('[MoonlitWolfManager] Failed to get canvas context');
                return;
            }
            
            // check if sprite sheets are loaded
            if (!this.loadedAnimations || Object.keys(this.loadedAnimations).length === 0) {
                this.log('[MoonlitWolfManager] Sprite sheets not yet loaded, trying to load now...');
                
                // try to load main image from the path in config
                const leftImgPath = this.getAbsoluteResourcePath(this.config.animations.left);
                const img = new Image();
                img.onload = () => {
                    this.log('[MoonlitWolfManager] Main image loaded for animation');
                ...
                img.onerror = (err) => {
                    this.error('[MoonlitWolfManager] Failed to load image for animation:', err);
                };
                img.src = leftImgPath;
                return;
            }
            ...
            this.error('[MoonlitWolfManager] Error in startSpriteAnimation:', error);
        }
    }
    ...
    initializeAnimation(element, canvas, ctx, isFlipped) {
        try {
            ...
            if (!element || !element.isConnected) {
                this.error('[MoonlitWolfManager] Element no longer connected to DOM');
                return;
            }
            
            const moonlitWolfData = this.animatedMoonlitWolves.get(element);
            if (!moonlitWolfData) {
                this.error('[MoonlitWolfManager] Moonlit wolf data not found in initializeAnimation');
                return;
            }
            
            // check if sprite sheets are loaded
            if (!this.loadedAnimations || Object.keys(this.loadedAnimations).length === 0) {
                this.error('[MoonlitWolfManager] Sprite sheets not loaded');
                return;
            }
            
            // at least one default image is required
            if (!this.loadedAnimations.default || !this.loadedAnimations.default.complete) {
                this.error('[MoonlitWolfManager] Default sprite sheet not loaded or incomplete');
                return;
            }
            
            // Clear any existing animation
            if (moonlitWolfData.animationInterval) {
                clearInterval(moonlitWolfData.animationInterval);
                moonlitWolfData.animationInterval = null;
            }
            ...
            if (canvas.width !== elementWidth * 1.2 || canvas.height !== elementHeight * 1.2) {
                this.log(`[MoonlitWolfManager] Updating canvas size to match element: ${elementWidth}x${elementHeight}`);
            ...
            const isMoving = element.classList.contains('pet-moving') || element.classList.contains('moonlit-wolf-moving');
            
            moonlitWolfData.currentFrame = 0;
            moonlitWolfData.animationState = isMoving ? 'walking' : 'idle';
            
            // immediately draw the first frame
            this.drawFrame(ctx, canvas, this.loadedAnimations, moonlitWolfData.currentFrame);
            
            // set animation interval
            const frameDuration = this.config.animationSettings.frameDuration;
            
            moonlitWolfData.animationInterval = setInterval(() => {
                // check if element is still connected to DOM
                if (!element.isConnected) {
                    if (moonlitWolfData.animationInterval) {
                        clearInterval(moonlitWolfData.animationInterval);
                ...
                const isCurrentlyMoving = element.classList.contains('pet-moving') || 
                                          element.classList.contains('moonlit-wolf-moving');
                
                // update animation based on current state
                if (isCurrentlyMoving) {
                    // when in moving state, play the complete walking animation
                    if (moonlitWolfData.animationState !== 'walking') {
                        // state changed from idle to moving, reset to the 0th frame
                        moonlitWolfData.currentFrame = 0;
                        moonlitWolfData.animationState = 'walking';
                    } else {
                        // loop within the walking frame range
                        const {start, end} = this.config.animationSettings.walkingFrames;
                        moonlitWolfData.currentFrame++;
                        
                        // loop back to the start frame
                        if (moonlitWolfData.currentFrame > end) {
                            moonlitWolfData.currentFrame = start;
                        }
                    }
                } else {
                    // when not in moving state, only display idle frame
                    if (moonlitWolfData.animationState !== 'idle') {
                        moonlitWolfData.currentFrame = this.config.animationSettings.idleFrames.start;
                        moonlitWolfData.animationState = 'idle';
                    }
                ...
                if (Math.abs(currentWidth - elementWidth) > elementWidth * 0.1 ||
                    ...
                    this.log(`[MoonlitWolfManager] Element size changed, updated canvas: ${canvas.width}x${canvas.height}`);
                }
            ...
            this.error('[MoonlitWolfManager] Error in initializeAnimation:', error);
        }
    }
    ...
    getCurrentRotationTestConfig() {
        ...
        if (now - this.rotationTestConfig.lastChangeTime > this.rotationTestConfig.timePerTest) {
            ...
            console.log(`[MoonlitWolfManager] Test rotation configuration #${this.rotationTestConfig.currentTest}: ${currentConfig.description}`);
        }
    ...
    drawFrame(ctx, canvas, spriteSheets, frameIndex) {
        ...
        if (!frameData) {
            this.error(`[MoonlitWolfManager] Frame data not found for: ${frameName}`);
            return;
        }
        ...
        try {
            ...
            this.error('[MoonlitWolfManager] Error drawing frame:', error);
        }
    }
    
    pauseAllAnimations() {
        this.animatedMoonlitWolves.forEach((moonlitWolfData, element) => {
            if (moonlitWolfData.animationInterval) {
                clearInterval(moonlitWolfData.animationInterval);
                moonlitWolfData.animationInterval = null;
            }
    ...
    resumeAllAnimations() {
        this.animatedMoonlitWolves.forEach((moonlitWolfData, element) => {
            if (!moonlitWolfData.animationInterval && element.isConnected) {
                // Restart animation
                const animContainer = element.querySelector('.moonlit-wolf-animation-container');
                if (animContainer) {
                    const canvas = animContainer.querySelector('.moonlit-wolf-animation-canvas');
                    if (canvas) {
                        this.startMoonlitWolfAnimation(element, moonlitWolfData.isFlipped);
                    }
                }
            }
        });
    ...
    cleanupNonExistentAnimations(currentNftIds) {
        ...
        elementsToRemove.forEach(element => {
            this.log(`[MoonlitWolfManager] Cleaning up animation for removed element: ${element.dataset.id || element.dataset.name}`);
    ...
    updateMoonlitWolfDirection(element, isRight) {
        this.log(`[MoonlitWolfManager] Updating moonlit wolf direction: ${isRight ? 'right' : 'left'}`);
        ...
        const container = element.querySelector('.moonlit-wolf-animation-container');
        if (container) {
            if (isRight) {
        ...
        const moonlitWolfData = this.animatedMoonlitWolves.get(element);
        if (moonlitWolfData) {
            moonlitWolfData.isFlipped = isRight;
            this.animatedMoonlitWolves.set(element, moonlitWolfData);
        }
    ...
    cleanupAnimation(element) {
        ...
        if (moonlitWolfData.positionObserver) {
            clearInterval(moonlitWolfData.positionObserver);
        }
        ...
        this.animatedMoonlitWolves.delete(element);
    }
    ...
    cleanupAllAnimations() {
        this.animatedMoonlitWolves.forEach((_, element) => {
            this.cleanupAnimation(element);
        });
    }
     ...
     */
    resetProcessedState() {
        this.log('[MoonlitWolfManager] Resetting processed state, will recheck all NFTs');
        this.processedNftIds.clear();
    ...
    setupDirectionChangeListener() {
        ...
        document.addEventListener('pet-direction-changed', (event) => {
            ...
            if (this.isMoonlitWolfNFTElement(element)) {
                this.updateMoonlitWolfDirection(element, direction === 'right');
            }
        });
        
        // if PetMovementManager is available, listen to its direction change event
        if (window.PetMovementManager) {
            this.log('[MoonlitWolfManager] Adding PetMovementManager direction change hook');
            ...
            if (typeof originalUpdateDirectionState === 'function') {
                window.PetMovementManager.updateDirectionState = (pet) => {
                    ...
                    // then check if it is a moonlit wolf and update direction
                    if (this.isMoonlitWolfNFTElement(pet.element)) {
                        // get direction information
                        const isFacingRight = pet.state.direction.x > 0;
                        
                        // delay update to ensure state has been updated by the original method
                        setTimeout(() => {
                            this.updateMoonlitWolfDirection(pet.element, isFacingRight);
                        }, 10);
                    }
                };
            }
...
// Create and initialize the moonlit wolf manager
const moonlitWolfManager = new MoonlitWolfManager();

// Initialize when DOM is ready
document.addEventListener('DOMContentLoaded', () => {
    moonlitWolfManager.init();
});

// Listen for NFT refresh events on the page
window.addEventListener('nftRefreshed', () => {
    if (moonlitWolfManager.initialized) {
        // When NFTs are refreshed, reset processed state and recheck
        moonlitWolfManager.resetProcessedState();
        // Delay slightly to ensure DOM has updated
        setTimeout(() => moonlitWolfManager.checkForMoonlitWolves(), 500);
    }
});

// Export the moonlit wolf manager for use in other modules
window.MoonlitWolfManager = moonlitWolfManager;

// Add the backward compatibility method checkForMoonlitWolves just like in egg.js
// First check if this is already using PetAnimationBase
if (MoonlitWolfManager.prototype instanceof PetAnimationBase) {
    /**
     * Backward compatibility method - calls checkForPets
     * @param {boolean} applyShadows - Whether to apply shadows
     */
    MoonlitWolfManager.prototype.checkForMoonlitWolves = function(applyShadows = true) {
