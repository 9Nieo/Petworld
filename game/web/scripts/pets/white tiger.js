// WhiteTigerManager class to handle all white tiger-related functionality
class WhiteTigerManager {
    constructor() {
        // Configuration settings
        this.config = {
            // Animation paths - using relative paths
            animations: {
                left: '../../resources/actions/White tiger/left.png',
                leftAtlas: '../../resources/actions/White tiger/left.atlas'
            },
            // Animation settings
            animationSettings: {
                frameDuration: 70,      // Duration of each frame in milliseconds
                loopDelay: 0,           // Delay between animation loops in milliseconds
                totalFrames: 27,        // total frames (0-28)
                walkingFrames: {         // walking animation frame range
                    start: 0,            // walking animation start frame
                    end: 26             // walking animation end frame
                },
            ...
            shadow: {
                ...
                applyToExisting: true    // Whether to apply to existing white tigers
            }
        };
        ...
        // Track animated white tigers
        this.animatedWhiteTigers = new Map();
    ...
    /**
     * Initialize the White Tiger Manager
     */
    async init() {
        if (this.initialized) return;
        
        this.log('[WhiteTigerManager] Initializing white tiger animation system...');
        
        // Load and parse the atlas file
        try {
            await this.loadAtlasData();
        } catch (error) {
            this.error('[WhiteTigerManager] Failed to load atlas data:', error);
        }
        
        // Listen for NFT loaded events to detect white tigers
        ...
        // Check for white tigers immediately if we're on the main page
        if (window.location.href.includes('home.html')) {
            // Short delay to ensure DOM is fully loaded
            setTimeout(() => this.checkForWhiteTigers(), 1000);
        }
        
        // Setup periodic white tiger checking on main page
        this.setupPeriodicWhiteTigerCheck();
    ...
    initShadowSystem() {
        ...
        this.log('[WhiteTigerManager] Initializing shadow system...');
        
        // Check if PetShadowManager is available
        if (window.PetShadowManager) {
            this.log('[WhiteTigerManager] PetShadowManager found, will use for white tiger shadows');
            
            if (this.config.shadow.applyToExisting) {
                setTimeout(() => {
                    this.applyAllWhiteTigerShadows();
                }, 1500);
            }
            
            document.addEventListener('white-tiger-moving-state-changed', (event) => {
                if (event.detail && event.detail.element) {
                    const whiteTigerElement = event.detail.element;
                    
                    // Update shadow state
                    if (window.PetShadowManager && window.PetShadowManager.updateShadow) {
                        window.PetShadowManager.updateShadow(whiteTigerElement);
                    }
                }
            });
        } else {
            this.log('[WhiteTigerManager] PetShadowManager not available, shadows will not be applied');
        }
    }
    
    applyAllWhiteTigerShadows() {
        if (!window.PetShadowManager) return;
        
        this.log('[WhiteTigerManager] Applying shadows to all white tigers...');
        
        const whiteTigerElements = document.querySelectorAll('.white-tiger-nft');
        let count = 0;
        
        whiteTigerElements.forEach(whiteTigerElement => {
            // Add shadow
            window.PetShadowManager.addShadow(whiteTigerElement, {
                ratio: 0.8,  // Shadow width relative to white tiger
                jumpEffect: true  // Enable jumping effect
            });
            count++;
        });
        
        this.log(`[WhiteTigerManager] Applied shadows to ${count} white tigers`);
    }
    ...
    async loadAtlasData() {
        ...
        try {
            ...
            this.log(`[WhiteTigerManager] Atlas path: ${atlasPath}`);
            ...
            this.log('[WhiteTigerManager] Atlas data parsed using SpriteAtlasHelper');
            
            // get all image files to load
            const imageFiles = this.atlasData.meta.images.map(img => img.file);
            this.log(`[WhiteTigerManager] Atlas references ${imageFiles.length} image files:`, imageFiles);
            ...
            this.log('[WhiteTigerManager] Atlas data and all images loaded successfully');
        } catch (error) {
            this.error('[WhiteTigerManager] Error loading atlas file:', error);
    ...
    async preloadAllAnimations(basePath, imageFiles) {
        try {
            ...
            this.log(`[WhiteTigerManager] Successfully loaded ${Object.keys(this.loadedAnimations).length - 1} sprite sheets`);
        } catch (error) {
            this.error('[WhiteTigerManager] Error preloading animations:', error);
    ...
    preloadAnimations() {
        ...
        if (!this.loadedAnimations || Object.keys(this.loadedAnimations).length === 0) {
            ...
            leftImg.onload = () => {
                this.log(`[WhiteTigerManager] Main image preloaded successfully`);
                this.loadedAnimations = { default: leftImg, 0: leftImg };
            };
            leftImg.onerror = (err) => {
                this.error(`[WhiteTigerManager] Error preloading main image:`, err);
            };
            
            // Get full path
            const imagePath = this.getAbsoluteResourcePath(this.config.animations.left);
            this.log(`[WhiteTigerManager] Main image path: ${imagePath}`);
    ...
    setupPeriodicWhiteTigerCheck() {
        if (window.location.href.includes('home.html')) {
            setInterval(() => {
                this.checkForWhiteTigers(false); // Pass false to indicate not to auto-apply shadows
            }, 10000);
        }
    }
    ...
    /**
     * Observe the NFT container for new white tigers
     */
    observeNFTContainer() {
        ...
        const observer = new MutationObserver((mutations) => {
            let shouldCheckWhiteTigers = false;
            
            mutations.forEach(mutation => {
                if (mutation.type === 'childList' && mutation.addedNodes.length > 0) {
                    shouldCheckWhiteTigers = true;
                }
            });
            
            if (shouldCheckWhiteTigers) {
                this.checkForWhiteTigers();
            }
        });
        ...
        this.log('[WhiteTigerManager] Now monitoring farm container for new NFTs');
    }
    ...
    createFallbackAtlasData() {
        this.log('[WhiteTigerManager] Creating fallback atlas data');
    ...
    handleNFTLoadedEvent(event) {
        ...
        if (!nftElement.isConnected) {
            this.log(`[WhiteTigerManager] NFT element not connected to DOM, skipping processing`);
            return;
        }
        ...
        if (this.processedNftIds.has(nftId)) {
            this.log(`[WhiteTigerManager] NFT already processed, skipping: ${nftId}`);
            return;
        }
        
        // If element already has animation, skip
        if (this.animatedWhiteTigers.has(nftElement)) {
            this.log(`[WhiteTigerManager] Element already animated, skipping: ${nftId}`);
            return;
        }
        
        // Check if this is a white tiger
        if (this.isWhiteTigerNFT(nftData)) {
            this.log('[WhiteTigerManager] White Tiger NFT detected from event:', nftData.metadata?.name || nftData.name || nftId);
            
            // Mark this NFT as processed
            this.processedNftIds.add(nftId);
            
            // Apply animation
            this.applyWhiteTigerAnimation(nftElement);
        }
    }
    
    /**
     * Check the page for white tiger NFTs and apply animations
     * @param {boolean} applyShadows - Whether to apply shadows, default is true
     */
    checkForWhiteTigers(applyShadows = true) {
        this.log('[WhiteTigerManager] Checking for white tigers on the page...');
        
        // Find all NFT elements in the farm
        const farmContainer = document.getElementById('farm-animals-container');
        if (!farmContainer) {
            this.error('[WhiteTigerManager] Farm container not found');
            return;
        }
        
        // Find all animal elements
        const animalElements = farmContainer.querySelectorAll('.animal');
        this.log(`[WhiteTigerManager] Found ${animalElements.length} animal elements`);
        ...
        animalElements.forEach((animalElement, index) => {
            ...
            if (this.animatedWhiteTigers.has(animalElement)) {
                this.log(`[WhiteTigerManager] Element already animated, skipping: ${nftId}`);
                return;
            }
            
            // Check if this is a white tiger
            if (this.isWhiteTigerNFTElement(animalElement)) {
                this.log(`[WhiteTigerManager] Found white tiger NFT #${index} (ID: ${nftId}):`, animalElement.dataset.name);
                
                // Mark this NFT as processed
                this.processedNftIds.add(nftId);
                
                // Apply animation
                this.applyWhiteTigerAnimation(animalElement);
            }
        });
    ...
    isWhiteTigerNFTElement(element) {
        // Check if the element has dataset
        if (!element.dataset) return false;
        
        // Check for white tiger name, type, or class
        const name = element.dataset.name || '';
        const type = element.dataset.type || '';
        
        const hasWhiteTigerName = name.toLowerCase().includes('white tiger');
        const hasWhiteTigerType = type.toLowerCase() === 'white tiger';
        const hasWhiteTigerClass = element.classList.contains('white-tiger-nft') || element.classList.contains('white tiger');
        
        return hasWhiteTigerName || hasWhiteTigerType || hasWhiteTigerClass;
    }
    
    isWhiteTigerNFT(nftData) {
        if (!nftData) return false;
        
        // Check metadata name
        if (nftData.metadata && nftData.metadata.name) {
            return nftData.metadata.name.toLowerCase().includes('white tiger');
        }
        
        // Check direct name property
        if (nftData.name) {
            return nftData.name.toLowerCase().includes('white tiger');
        }
        
        // Check any other properties that could identify a white tiger
        if (nftData.type) {
            return nftData.type.toLowerCase() === 'white tiger';
        }
        
        return false;
    }

    applyWhiteTigerAnimation(element) {
        if (this.animatedWhiteTigers.has(element)) return;
        
        this.log('[WhiteTigerManager] Applying white tiger animation to element:', element);
        
        // Check if element is visible and has dimensions
        if (!element.isConnected) {
            this.error('[WhiteTigerManager] Element is not connected to DOM, cannot animate:', element);
            return;
        }
        ...
        if (window.PetSizeHelper) {
            window.PetSizeHelper.applyAnimationWithSizeCheck(
                element,
                'white tiger',
                (el) => this.doApplyWhiteTigerAnimation(el),
                {
                    maxWaitAttempts: 8,
                    fallbackSize: { width: '65px', height: '65px' }
                }
            );
        } else {
            // Fallback to original logic if PetSizeHelper is not available
            this.log('[WhiteTigerManager] PetSizeHelper not available, using fallback logic');
            
            // Check element dimensions, delay processing if zero
            if (element.offsetWidth === 0 || element.offsetHeight === 0) {
                this.log('[WhiteTigerManager] Element has zero dimensions, delaying animation:', element);
                ...
                setTimeout(() => {
                    ...
                    if (element.isConnected && element.offsetWidth > 0 && element.offsetHeight > 0) {
                        this.log('[WhiteTigerManager] Element now has dimensions, applying animation');
                        this.doApplyWhiteTigerAnimation(element);
                    } else {
                        this.error('[WhiteTigerManager] Element still has zero dimensions after delay, applying PetSizeManager or default size');
                        ...
                        if (window.PetSizeManager) {
                            ...
                            this.log('[WhiteTigerManager] Applied PetSizeManager sizing');
                        }
                        
                        // Check if PetSizeManager worked
                        if (element.offsetWidth === 0 || element.offsetHeight === 0) {
                            // Fallback to minimum size
                            element.style.setProperty('width', '65px', 'important');
                            element.style.setProperty('height', '65px', 'important');
                            this.log('[WhiteTigerManager] Applied fallback size as last resort');
                        }
    ...
    /**
     * Actual method to apply white tiger animation (separated to support delayed processing)
     * @param {HTMLElement} element - The NFT element to animate
     */
    doApplyWhiteTigerAnimation(element) {
        ...
        element.classList.add('white-tiger-nft');
        
        this.animatedWhiteTigers.set(element, {
            element,
            isFlipped,
        ...
        if (!success) {
            this.error('[WhiteTigerManager] Failed to prepare element for animation');
            return;
        }
        
        // start animation
        this.startWhiteTigerAnimation(element, isFlipped);
        
        // apply shadow - ensure shadow is applied before setting movement
        if (this.config.shadow.enabled) {
            this.applyWhiteTigerShadow(element);
        }
        ...
        this.updateWhiteTigerDirection(element, isFlipped);
        
        this.log(`[WhiteTigerManager] White tiger animation applied for: ${element.dataset.name || 'unnamed white tiger'}`);
    }
    
    applyWhiteTigerShadow(element) {
        if (!this.config.shadow.enabled || !window.PetShadowManager) return;
        
        // If already in tracking map, check if already has shadow
        const whiteTigerData = this.animatedWhiteTigers.get(element);
        if (whiteTigerData && whiteTigerData.hasShadow) {
            this.log('[WhiteTigerManager] White tiger already has shadow, skipping:', element.dataset.name || element.dataset.id || 'unnamed');
            return;
        }
        
        // Add white tiger-nft class to support shadow selector
        element.classList.add('white-tiger-nft');
        ...
        if (whiteTigerData) {
            whiteTigerData.hasShadow = true;
            this.animatedWhiteTigers.set(element, whiteTigerData);
        }
        
        this.log('[WhiteTigerManager] Applied shadow to white tiger:', element.dataset.name || element.dataset.id || 'unnamed');
    }
    
    /**
     * Enable movement for white tigers
     * @param {HTMLElement} element - White tiger element
     */
    enableMovement(element) {
        if (!window.PetMovementManager) {
            this.log('[WhiteTigerManager] PetMovementManager not available, cannot enable movement');
            return;
        }
        ...
        const positionObserver = setInterval(() => {
            ...
            if (Math.abs(deltaX) > 0.5) {
                ...
                this.updateWhiteTigerDirection(element, isMovingRight);
                lastX = currentX;
            }
        }, 100); 
        
        const whiteTigerData = this.animatedWhiteTigers.get(element) || {};
        whiteTigerData.positionObserver = positionObserver;
        this.animatedWhiteTigers.set(element, whiteTigerData);
        ...
        window.PetMovementManager.enableMovement(element, {
            ...
            onStateChange: (isMoving) => {
                ...
                const whiteTigerEvent = new CustomEvent('white-tiger-moving-state-changed', {
                    detail: {
                        element: element,
                        isMoving: isMoving
                    },
        ...
        const directionHandler = (event) => {
            if (event.detail && event.detail.element === element) {
                ...
                this.updateWhiteTigerDirection(element, isRight);
            }
        };
        ...
        this.log(`[WhiteTigerManager] Enabled movement for white tiger: ${element.dataset.name || 'unnamed'} with speed ${speed.toFixed(2)}`);
    }
    ...
    prepareElementForAnimation(element, isFlipped) {
        ...
        if (!element || !element.isConnected) {
            this.error('[WhiteTigerManager] Cannot prepare element - element is not connected to DOM');
            return false;
        }
        ...
        this.log(`[White TigerManager] Element dimension check: offset(${offsetWidth}x${offsetHeight}), css(${cssWidth}x${cssHeight})`);
        
        // Apply size using PetSizeManager first if available
        if (window.PetSizeManager) {
            window.PetSizeManager.setSize(element, 'white tiger');
            this.log('[White TigerManager] Applied size using PetSizeManager');
        }
        ...
        if (effectiveWidth === 0 || effectiveHeight === 0) {
            this.error('[White TigerManager] Element still has zero effective dimensions after PetSizeManager, applying fallback size');
        ...
        try {
            ...
            const oldContainer = element.querySelector('.white-tiger-animation-container');
            if (oldContainer) {
            ...
            animContainer.className = 'white-tiger-animation-container';
            ...
            animContainer.style.zIndex = '10'; // Show above the white tiger image
            ...
            if (shouldFlip) {
                this.log('[WhiteTigerManager] Setting initial animation direction: right (flipped)');
                ...
                this.log('[WhiteTigerManager] Setting initial animation direction: left (normal)');
            ...
            this.log(`[WhiteTigerManager] Element dimensions: ${elementWidth}x${elementHeight}`); 
            
            // Create canvas for sprite animation
            const canvas = document.createElement('canvas');
            canvas.className = 'white-tiger-animation-canvas';
            ...
            this.error('[WhiteTigerManager] Error preparing element for animation:', error);
            return false;
        }
    }
    
    /**
     * Start the white tiger animation
     * @param {HTMLElement} element - The element to animate
     * @param {boolean} isFlipped - Whether the animation is flipped
     */
    startWhiteTigerAnimation(element, isFlipped) {
        // Check element
        if (!element || !element.isConnected) {
            this.error('[WhiteTigerManager] Cannot start animation - element is not connected to DOM');
            return;
        }
        
        try {
            // Get animation container
            const animContainer = element.querySelector('.white-tiger-animation-container');
            if (!animContainer) {
                this.error('[WhiteTigerManager] Animation container not found');
                return;
            }
            
            // Get the canvas
            const canvas = animContainer.querySelector('.white-tiger-animation-canvas');
            if (!canvas) {
                this.error('[WhiteTigerManager] Canvas not found');
                return;
            }
            
            const whiteTigerData = this.animatedWhiteTigers.get(element);
            if (!whiteTigerData) {
                this.error('[WhiteTigerManager] White tiger data not found');
                return;
            }
            
            // Wait until the atlas data is loaded
            if (!this.atlasData) {
                this.log('[WhiteTigerManager] Atlas data not loaded yet, creating fallback');
                this.createFallbackAtlasData();
            }
            ...
            this.error('[WhiteTigerManager] Error starting white tiger animation:', error);
        }
    }
    ...
    startSpriteAnimation(element, canvas, isFlipped) {
        try {
            // Get white tiger data from tracking map
            const whiteTigerData = this.animatedWhiteTigers.get(element);
            if (!whiteTigerData) return;
            
            // Get the canvas context
            const ctx = canvas.getContext('2d');
            if (!ctx) {
                this.error('[WhiteTigerManager] Failed to get canvas context');
                return;
            }
            
            // check if sprite sheets are loaded
            if (!this.loadedAnimations || Object.keys(this.loadedAnimations).length === 0) {
                this.log('[WhiteTigerManager] Sprite sheets not yet loaded, trying to load now...');
                
                // try to load main image from the path in config
                const leftImgPath = this.getAbsoluteResourcePath(this.config.animations.left);
                const img = new Image();
                img.onload = () => {
                    this.log('[WhiteTigerManager] Main image loaded for animation');
                ...
                img.onerror = (err) => {
                    this.error('[WhiteTigerManager] Failed to load image for animation:', err);
                };
                img.src = leftImgPath;
                return;
            }
            ...
            this.error('[WhiteTigerManager] Error in startSpriteAnimation:', error);
        }
    }
    ...
    initializeAnimation(element, canvas, ctx, isFlipped) {
        try {
            ...
            if (!element || !element.isConnected) {
                this.error('[WhiteTigerManager] Element no longer connected to DOM');
                return;
            }
            
            const whiteTigerData = this.animatedWhiteTigers.get(element);
            if (!whiteTigerData) {
                this.error('[WhiteTigerManager] White tiger data not found in initializeAnimation');
                return;
            }
            
            // check if sprite sheets are loaded
            if (!this.loadedAnimations || Object.keys(this.loadedAnimations).length === 0) {
                this.error('[WhiteTigerManager] Sprite sheets not loaded');
                return;
            }
            
            // at least one default image is required
            if (!this.loadedAnimations.default || !this.loadedAnimations.default.complete) {
                this.error('[WhiteTigerManager] Default sprite sheet not loaded or incomplete');
                return;
            }
            
            // Clear any existing animation
            if (whiteTigerData.animationInterval) {
                clearInterval(whiteTigerData.animationInterval);
                whiteTigerData.animationInterval = null;
            }
            ...
            if (canvas.width !== elementWidth * 1.2 || canvas.height !== elementHeight * 1.2) {
                this.log(`[WhiteTigerManager] Updating canvas size to match element: ${elementWidth}x${elementHeight}`);
            ...
            const isMoving = element.classList.contains('pet-moving') || element.classList.contains('white-tiger-moving');
            
            whiteTigerData.currentFrame = 0;
            whiteTigerData.animationState = isMoving ? 'walking' : 'idle';
            
            // immediately draw the first frame
            this.drawFrame(ctx, canvas, this.loadedAnimations, whiteTigerData.currentFrame);
            
            // set animation interval
            const frameDuration = this.config.animationSettings.frameDuration;
            
            whiteTigerData.animationInterval = setInterval(() => {
                // check if element is still connected to DOM
                if (!element.isConnected) {
                    if (whiteTigerData.animationInterval) {
                        clearInterval(whiteTigerData.animationInterval);
                ...
                const isCurrentlyMoving = element.classList.contains('pet-moving') || 
                                          element.classList.contains('white-tiger-moving');
                
                // update animation based on current state
                if (isCurrentlyMoving) {
                    // when in moving state, play the complete walking animation
                    if (whiteTigerData.animationState !== 'walking') {
                        // state changed from idle to moving, reset to the 0th frame
                        whiteTigerData.currentFrame = 0;
                        whiteTigerData.animationState = 'walking';
                    } else {
                        // loop within the walking frame range
                        const {start, end} = this.config.animationSettings.walkingFrames;
                        whiteTigerData.currentFrame++;
                        
                        // loop back to the start frame
                        if (whiteTigerData.currentFrame > end) {
                            whiteTigerData.currentFrame = start;
                        }
                    }
                } else {
                    // when not in moving state, only display idle frame
                    if (whiteTigerData.animationState !== 'idle') {
                        whiteTigerData.currentFrame = this.config.animationSettings.idleFrames.start;
                        whiteTigerData.animationState = 'idle';
                    }
                ...
                if (Math.abs(currentWidth - elementWidth) > elementWidth * 0.1 ||
                    ...
                    this.log(`[WhiteTigerManager] Element size changed, updated canvas: ${canvas.width}x${canvas.height}`);
                }
            ...
            this.error('[WhiteTigerManager] Error in initializeAnimation:', error);
        }
    }
    ...
    getCurrentRotationTestConfig() {
        ...
        if (now - this.rotationTestConfig.lastChangeTime > this.rotationTestConfig.timePerTest) {
            ...
            console.log(`[WhiteTigerManager] Test rotation configuration #${this.rotationTestConfig.currentTest}: ${currentConfig.description}`);
        }
    ...
    drawFrame(ctx, canvas, spriteSheets, frameIndex) {
        ...
        if (!frameData) {
            this.error(`[WhiteTigerManager] Frame data not found for: ${frameName}`);
            return;
        }
        ...
        try {
            ...
            this.error('[WhiteTigerManager] Error drawing frame:', error);
        }
    }
    
    pauseAllAnimations() {
        this.animatedWhiteTigers.forEach((whiteTigerData, element) => {
            if (whiteTigerData.animationInterval) {
                clearInterval(whiteTigerData.animationInterval);
                whiteTigerData.animationInterval = null;
            }
    ...
    resumeAllAnimations() {
        this.animatedWhiteTigers.forEach((whiteTigerData, element) => {
            if (!whiteTigerData.animationInterval && element.isConnected) {
                // Restart animation
                const animContainer = element.querySelector('.white-tiger-animation-container');
                if (animContainer) {
                    const canvas = animContainer.querySelector('.white-tiger-animation-canvas');
                    if (canvas) {
                        this.startWhiteTigerAnimation(element, whiteTigerData.isFlipped);
                    }
                }
            }
        });
    ...
    cleanupNonExistentAnimations(currentNftIds) {
        ...
        elementsToRemove.forEach(element => {
            this.log(`[WhiteTigerManager] Cleaning up animation for removed element: ${element.dataset.id || element.dataset.name}`);
    ...
    updateWhiteTigerDirection(element, isRight) {
        this.log(`[WhiteTigerManager] Updating white tiger direction: ${isRight ? 'right' : 'left'}`);
        ...
        const container = element.querySelector('.white-tiger-animation-container');
        if (container) {
            if (isRight) {
        ...
        const whiteTigerData = this.animatedWhiteTigers.get(element);
        if (whiteTigerData) {
            whiteTigerData.isFlipped = isRight;
            this.animatedWhiteTigers.set(element, whiteTigerData);
        }
    ...
    cleanupAnimation(element) {
        ...
        if (whiteTigerData.positionObserver) {
            clearInterval(whiteTigerData.positionObserver);
        }
        ...
        this.animatedWhiteTigers.delete(element);
    }
    ...
    cleanupAllAnimations() {
        this.animatedWhiteTigers.forEach((_, element) => {
            this.cleanupAnimation(element);
        });
    }
     ...
     */
    resetProcessedState() {
        this.log('[WhiteTigerManager] Resetting processed state, will recheck all NFTs');
        this.processedNftIds.clear();
    ...
    setupDirectionChangeListener() {
        ...
        document.addEventListener('pet-direction-changed', (event) => {
            ...
            if (this.isWhiteTigerNFTElement(element)) {
                this.updateWhiteTigerDirection(element, direction === 'right');
            }
        });
        
        // if PetMovementManager is available, listen to its direction change event
        if (window.PetMovementManager) {
            this.log('[WhiteTigerManager] Adding PetMovementManager direction change hook');
            ...
            if (typeof originalUpdateDirectionState === 'function') {
                window.PetMovementManager.updateDirectionState = (pet) => {
                    ...
                    // then check if it is a white tiger and update direction
                    if (this.isWhiteTigerNFTElement(pet.element)) {
                        // get direction information
                        const isFacingRight = pet.state.direction.x > 0;
                        
                        // delay update to ensure state has been updated by the original method
                        setTimeout(() => {
                            this.updateWhiteTigerDirection(pet.element, isFacingRight);
                        }, 10);
                    }
                };
            }
...
// Create and initialize the white tiger manager
const whiteTigerManager = new WhiteTigerManager();

// Initialize when DOM is ready
document.addEventListener('DOMContentLoaded', () => {
    whiteTigerManager.init();
});

// Listen for NFT refresh events on the page
window.addEventListener('nftRefreshed', () => {
    if (whiteTigerManager.initialized) {
        // When NFTs are refreshed, reset processed state and recheck
        whiteTigerManager.resetProcessedState();
        // Delay slightly to ensure DOM has updated
        setTimeout(() => whiteTigerManager.checkForWhiteTigers(), 500);
    }
});

// Export the white tiger manager for use in other modules
window.WhiteTigerManager = whiteTigerManager;

// Add the backward compatibility method checkForWhiteTigers just like in white tiger.js
// First check if this is already using PetAnimationBase
if (WhiteTigerManager.prototype instanceof PetAnimationBase) {
    /**
     * Backward compatibility method - calls checkForPets
     * @param {boolean} applyShadows - Whether to apply shadows
     */
    WhiteTigerManager.prototype.checkForWhiteTigers = function(applyShadows = true) {
